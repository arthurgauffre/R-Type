...
struct call_traits<
    ...
    enable_if_t<!require_member<typename Impl::template proxy<T>::type,
                                Property>::is_valid>,
    enable_if_t<!require_free<T, Property>::is_valid>,
    enable_if_t<prefer_member<typename Impl::template proxy<T>::type,
                              Property>::is_valid>>
...
struct impl {
  template <typename T> struct proxy {
    ...
    struct type {
      ...
      template <typename P>
      auto prefer(P &&p) noexcept(noexcept(
          declval<conditional_t<true, T, P>>().prefer(static_cast<P &&>(p))))
          -> decltype(declval<conditional_t<true, T, P>>().prefer(
              static_cast<P &&>(p)));
    };
...
static constexpr const asio_prefer_fn::impl &prefer =
    asio_prefer_fn::static_instance<>::instance;
...
template <typename T, typename... Properties>
struct can_prefer
    : integral_constant<bool, asio_prefer_fn::call_traits<
