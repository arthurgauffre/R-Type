  ...
  if (handle_) {
    ...
    if (cb_userdata) {
      detail::password_callback_base *callback =
          static_cast<detail::password_callback_base *>(cb_userdata);
      delete callback;
#if ((OPENSSL_VERSION_NUMBER >= 0x10100000L) &&                                \
     (!defined(LIBRESSL_VERSION_NUMBER) ||                                     \
      LIBRESSL_VERSION_NUMBER >= 0x2070000fL)) ||                              \
...
ASIO_SYNC_OP_VOID context::clear_options(context::options o,
                                         asio::error_code &ec) {
...
ASIO_SYNC_OP_VOID context::set_options(context::options o,
                                       asio::error_code &ec) {
#if !defined(SSL_OP_NO_COMPRESSION)
  if ((o & context::no_compression) != 0) {
#if (OPENSSL_VERSION_NUMBER >= 0x00908000L)
...
ASIO_SYNC_OP_VOID context::set_verify_mode(verify_mode v,
                                           asio::error_code &ec) {
...
ASIO_SYNC_OP_VOID context::set_verify_depth(int depth, asio::error_code &ec) {
...
ASIO_SYNC_OP_VOID context::load_verify_file(const std::string &filename,
                                            asio::error_code &ec) {
  ::ERR_clear_error();
...
ASIO_SYNC_OP_VOID context::add_certificate_authority(const const_buffer &ca,
                                                     asio::error_code &ec) {
  ::ERR_clear_error();
...
ASIO_SYNC_OP_VOID context::set_default_verify_paths(asio::error_code &ec) {
  ::ERR_clear_error();
...
ASIO_SYNC_OP_VOID context::add_verify_path(const std::string &path,
                                           asio::error_code &ec) {
  ::ERR_clear_error();
...
void context::use_certificate(const const_buffer &certificate,
                              file_format format) {
  asio::error_code ec;
...
ASIO_SYNC_OP_VOID context::use_certificate(const const_buffer &certificate,
  ...
  if (format == context_base::asn1) {
    if (::SSL_CTX_use_certificate_ASN1(
            handle_, static_cast<int>(certificate.size()),
...
ASIO_SYNC_OP_VOID context::use_certificate_file(const std::string &filename,
                                                file_format format,
                                                asio::error_code &ec) {
  int file_type;
  switch (format) {
  ...
  if (::SSL_CTX_use_certificate_file(handle_, filename.c_str(), file_type) !=
      1) {
    ec = translate_error(::ERR_get_error());
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
...
ASIO_SYNC_OP_VOID context::use_certificate_chain(const const_buffer &chain,
                                                 asio::error_code &ec) {
  ::ERR_clear_error();
  ...
  if (bio.p) {
...
#endif // (OPENSSL_VERSION_NUMBER >= 0x10100000L)
    x509_cleanup cert = {
        ::PEM_read_bio_X509_AUX(bio.p, 0, callback, cb_userdata)};
    if (!cert.p) {
    ...
    while (X509 *cacert =
               ::PEM_read_bio_X509(bio.p, 0, callback, cb_userdata)) {
...
ASIO_SYNC_OP_VOID
context::use_certificate_chain_file(const std::string &filename,
                                    asio::error_code &ec) {
  ::ERR_clear_error();
...
void context::use_private_key(const const_buffer &private_key,
...
ASIO_SYNC_OP_VOID context::use_private_key(const const_buffer &private_key,
                                           context::file_format format,
                                           asio::error_code &ec) {
  ::ERR_clear_error();
  ...
  if (bio.p) {
    ...
    case context_base::pem:
      evp_private_key.p =
          ::PEM_read_bio_PrivateKey(bio.p, 0, callback, cb_userdata);
      break;
    default: {
      ec = asio::error::invalid_argument;
      ASIO_SYNC_OP_VOID_RETURN(ec);
...
void context::use_rsa_private_key(const const_buffer &private_key,
                                  context::file_format format) {
  asio::error_code ec;
...
ASIO_SYNC_OP_VOID context::use_rsa_private_key(const const_buffer &private_key,
                                               context::file_format format,
                                               asio::error_code &ec) {
  ::ERR_clear_error();
  ...
  if (bio.p) {
    ...
    case context_base::pem:
      rsa_private_key.p =
          ::PEM_read_bio_RSAPrivateKey(bio.p, 0, callback, cb_userdata);
      break;
    default: {
      ec = asio::error::invalid_argument;
      ASIO_SYNC_OP_VOID_RETURN(ec);
...
ASIO_SYNC_OP_VOID context::use_private_key_file(const std::string &filename,
                                                context::file_format format,
                                                asio::error_code &ec) {
  int file_type;
  switch (format) {
  ...
  if (::SSL_CTX_use_PrivateKey_file(handle_, filename.c_str(), file_type) !=
      1) {
    ec = translate_error(::ERR_get_error());
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
...
void context::use_rsa_private_key_file(const std::string &filename,
                                       context::file_format format) {
  asio::error_code ec;
...
ASIO_SYNC_OP_VOID context::use_rsa_private_key_file(const std::string &filename,
                                                    context::file_format format,
                                                    asio::error_code &ec) {
#if (OPENSSL_VERSION_NUMBER >= 0x30000000L)
  ::ERR_clear_error();
  ...
  if (::SSL_CTX_use_RSAPrivateKey_file(handle_, filename.c_str(), file_type) !=
      1) {
    ec = translate_error(::ERR_get_error());
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
...
ASIO_SYNC_OP_VOID context::use_tmp_dh(const const_buffer &dh,
                                      asio::error_code &ec) {
  ::ERR_clear_error();
...
ASIO_SYNC_OP_VOID context::use_tmp_dh_file(const std::string &filename,
                                           asio::error_code &ec) {
  ::ERR_clear_error();
...
ASIO_SYNC_OP_VOID context::do_use_tmp_dh(BIO *bio, asio::error_code &ec) {
  ::ERR_clear_error();
...
ASIO_SYNC_OP_VOID
context::do_set_verify_callback(detail::verify_callback_base *callback,
                                asio::error_code &ec) {
  if (SSL_CTX_get_app_data(handle_)) {
  ...
  ::SSL_CTX_set_verify(handle_, ::SSL_CTX_get_verify_mode(handle_),
...
ASIO_SYNC_OP_VOID
context::do_set_password_callback(detail::password_callback_base *callback,
                                  asio::error_code &ec) {
#if ((OPENSSL_VERSION_NUMBER >= 0x10100000L) &&                                \
     (!defined(LIBRESSL_VERSION_NUMBER) ||                                     \
      LIBRESSL_VERSION_NUMBER >= 0x2070000fL)) ||                              \
  ...
  if (old_callback)
    delete static_cast<detail::password_callback_base *>(old_callback);
...
int context::password_callback_function(char *buf, int size, int purpose,
                                        void *data) {
  ...
  if (data) {
    ...
    std::string passwd = callback->call(static_cast<std::size_t>(size),
                                        purpose ? context_base::for_writing
...
BIO *context::make_buffer_bio(const const_buffer &b) {
  return ::BIO_new_mem_buf(const_cast<void *>(b.data()),
...
asio::error_code context::translate_error(long error) {
  ...
  if (ERR_SYSTEM_ERROR(error)) {
    return asio::error_code(static_cast<int>(ERR_GET_REASON(error)),
