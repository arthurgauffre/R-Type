...
struct prefer_only_is_preferable<InnerProperty,
                                 enable_if_t<InnerProperty::is_preferable>> {
...
struct prefer_only_property<
    InnerProperty,
    void_t<decltype(asio::declval<const InnerProperty>().value())>> {
  InnerProperty property;
...
char prefer_only_value_memfn_helper(
    prefer_only_memfns_check<void (prefer_only_memfns_base::*)(),
...
struct prefer_only
  ...
  friend constexpr prefer_result_t<const Executor &, const InnerProperty &>
  prefer(const Executor &ex, const prefer_only<Property> &p,
         enable_if_t<is_same<Property, InnerProperty>::value> * = 0,
         enable_if_t<can_prefer<const Executor &, const InnerProperty &>::value>
             * = 0)
  ...
  template <typename Executor, typename Property>
  friend constexpr query_result_t<const Executor &, const InnerProperty &>
  query(const Executor &ex, const prefer_only<Property> &p,
        enable_if_t<is_same<Property, InnerProperty>::value> * = 0,
        enable_if_t<can_query<const Executor &, const InnerProperty &>::value>
            * = 0)
#if !defined(ASIO_MSVC) &&                                                     \
    !defined(__clang__) // Clang crashes if noexcept is used here.
...
struct prefer_free_default<
    T, execution::prefer_only<InnerProperty>,
