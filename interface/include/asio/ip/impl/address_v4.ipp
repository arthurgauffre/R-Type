...
std::string address_v4::to_string() const {
  ...
  const char *addr = asio::detail::socket_ops::inet_ntop(
      ASIO_OS_DEF(AF_INET), &addr_, addr_str, asio::detail::max_addr_v4_str_len,
      0, ec);
  if (addr == 0)
...
bool address_v4::is_unspecified() const noexcept { return to_uint() == 0; }

#if !defined(ASIO_NO_DEPRECATED)
bool address_v4::is_class_a() const { return (to_uint() & 0x80000000) == 0; }
...
address_v4 make_address_v4(const char *str, asio::error_code &ec) noexcept {
  address_v4::bytes_type bytes;
  if (asio::detail::socket_ops::inet_pton(ASIO_OS_DEF(AF_INET), str, &bytes, 0,
                                          ec) <= 0)
    return address_v4();
...
address_v4 make_address_v4(string_view str, asio::error_code &ec) noexcept {
