...
public:
  ...
  public:
    per_timer_data()
        : heap_index_((std::numeric_limits<std::size_t>::max)()), next_(0),
          prev_(0) {}
  ...
  virtual long wait_duration_msec(long max_duration) const {
    ...
    return this->to_msec(Time_Traits::to_posix_duration(Time_Traits::subtract(
                             heap_[0].time_, Time_Traits::now())),
                         max_duration);
  }
  ...
  virtual long wait_duration_usec(long max_duration) const {
    ...
    return this->to_usec(Time_Traits::to_posix_duration(Time_Traits::subtract(
                             heap_[0].time_, Time_Traits::now())),
                         max_duration);
  }
  ...
  void cancel_timer_by_key(per_timer_data *timer, op_queue<operation> &ops,
...
private:
  ...
  void remove_timer(per_timer_data &timer) {
    ...
    if (!heap_.empty() && index < heap_.size()) {
      if (index == heap_.size() - 1) {
        ...
        timer.heap_index_ = (std::numeric_limits<std::size_t>::max)();
        heap_.pop_back();
        if (index > 0 && Time_Traits::less_than(heap_[index].time_,
  ...
  static bool
  is_positive_infinity(const boost::date_time::base_time<T, TimeSystem> &time) {
