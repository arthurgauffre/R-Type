...
public:
  explicit thread_function(select_reactor *r) : this_(r) {}

  void operator()() { this_->run_thread(); }
...
select_reactor::select_reactor(asio::execution_context &ctx)
    : execution_context_service_base<select_reactor>(ctx),
      scheduler_(use_service<scheduler_type>(ctx)), mutex_(), interrupter_(),
#if defined(ASIO_HAS_IOCP)
      stop_thread_(false), thread_(0), restart_reactor_(this),
#endif // defined(ASIO_HAS_IOCP)
      shutdown_(false) {
#if defined(ASIO_HAS_IOCP)
...
select_reactor::~select_reactor() { shutdown(); }
...
void select_reactor::notify_fork(asio::execution_context::fork_event fork_ev) {
#if defined(ASIO_HAS_IOCP)
  (void)fork_ev;
#else  // defined(ASIO_HAS_IOCP)
...
void select_reactor::init_task() { scheduler_.init_task(); }
...
int select_reactor::register_internal_descriptor(
    int op_type, socket_type descriptor, select_reactor::per_descriptor_data &,
    reactor_op *op) {
  asio::detail::mutex::scoped_lock lock(mutex_);
...
void select_reactor::call_post_immediate_completion(operation *op,
...
void select_reactor::deregister_descriptor(
    socket_type descriptor, select_reactor::per_descriptor_data &, bool) {
  asio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, asio::error::operation_aborted);
}
...
void select_reactor::run(long usec, op_queue<operation> &ops) {
  ...
  asio::error_code ec;
  int retval =
      socket_ops::select(static_cast<int>(max_fd + 1), fd_sets_[read_op],
                         fd_sets_[write_op], fd_sets_[except_op], tv, ec);
...
void select_reactor::interrupt() { interrupter_.interrupt(); }
...
void select_reactor::restart_reactor::do_complete(
    void *owner, operation *base, const asio::error_code & /*ec*/,
  ...
  if (owner) {
    ...
    asio::detail::signal_blocker sb;
    reactor->thread_ = new asio::detail::thread(thread_function(reactor));
  }
}
#endif // defined(ASIO_HAS_IOCP)
...
void select_reactor::cancel_ops_unlocked(socket_type descriptor,
  ...
  for (int i = 0; i < max_ops; ++i)
    need_interrupt =
        op_queue_[i].cancel_operations(descriptor, ops, ec) || need_interrupt;
  scheduler_.post_deferred_completions(ops);
  if (need_interrupt)
    interrupter_.interrupt();
}
