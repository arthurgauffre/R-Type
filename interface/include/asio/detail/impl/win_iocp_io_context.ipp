...
struct win_iocp_io_context::thread_function {
  explicit thread_function(win_iocp_io_context *s) : this_(s) {}
  ...
  void operator()() {
    while (::InterlockedExchangeAdd(&io_context_->shutdown_, 0) == 0) {
      if (::WaitForSingleObject(io_context_->waitable_timer_.handle,
        ...
        ::PostQueuedCompletionStatus(io_context_->iocp_.handle, 0,
                                     wake_for_dispatch, 0);
      }
    }
  }
...
win_iocp_io_context::win_iocp_io_context(asio::execution_context &ctx,
                                         int concurrency_hint, bool own_thread)
    : execution_context_service_base<win_iocp_io_context>(ctx), iocp_(),
      outstanding_work_(0), stopped_(0), stop_event_posted_(0), shutdown_(0),
      gqcs_timeout_(get_gqcs_timeout()), dispatch_required_(0),
      concurrency_hint_(concurrency_hint) {
  ASIO_HANDLER_TRACKING_INIT;

  iocp_.handle = ::CreateIoCompletionPort(
...
asio::error_code win_iocp_io_context::register_handle(HANDLE handle,
                                                      asio::error_code &ec) {
...
void win_iocp_io_context::on_pending(win_iocp_operation *op) {
  if (::InterlockedCompareExchange(&op->ready_, 1, 0) == 1) {
    // Enqueue the operation on the I/O completion port.
    if (!::PostQueuedCompletionStatus(iocp_.handle, 0,
                                      overlapped_contains_result, op)) {
      // Out of resources. Put on completed queue instead.
      mutex::scoped_lock lock(dispatch_mutex_);
      completed_ops_.push(op);
      ::InterlockedExchange(&dispatch_required_, 1);
...
void win_iocp_io_context::on_completion(win_iocp_operation *op,
  ...
  op->Internal =
      reinterpret_cast<ulong_ptr_t>(&asio::error::get_system_category());
  op->Offset = last_error;
  op->OffsetHigh = bytes_transferred;

  // Enqueue the operation on the I/O completion port.
  if (!::PostQueuedCompletionStatus(iocp_.handle, 0, overlapped_contains_result,
                                    op)) {
    // Out of resources. Put on completed queue instead.
    mutex::scoped_lock lock(dispatch_mutex_);
    completed_ops_.push(op);
...
size_t win_iocp_io_context::do_one(DWORD msec,
  ...
  for (;;) {
    ...
    BOOL ok = ::GetQueuedCompletionStatus(
        iocp_.handle, &bytes_transferred, &completion_key, &overlapped,
    ...
    if (overlapped) {
      ...
      if (completion_key == overlapped_contains_result) {
        result_ec = asio::error_code(
            static_cast<int>(op->Offset),
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base &queue) {
  ...
  if (!waitable_timer_.handle) {
    ...
    timeout.QuadPart *= 10;
    ::SetWaitableTimer(waitable_timer_.handle, &timeout, max_timeout_msec, 0, 0,
                       FALSE);
  }
...
void win_iocp_io_context::update_timeout() {
  if (timer_thread_.get()) {
    ...
    if (timeout_usec < max_timeout_usec) {
      ...
      timeout.QuadPart *= 10;
      ::SetWaitableTimer(waitable_timer_.handle, &timeout, max_timeout_msec, 0,
                         0, FALSE);
    }
  }
}
