...
class win_iocp_handle_service::overlapped_wrapper : public OVERLAPPED {
public:
...
win_iocp_handle_service::win_iocp_handle_service(execution_context &context)
    : execution_context_service_base<win_iocp_handle_service>(context),
      ...
      nt_set_info_(0), mutex_(), impl_list_(0) {}
...
asio::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type &impl, asio::error_code &ec) {
  if (is_open(impl)) {
    ASIO_HANDLER_OPERATION((iocp_service_.context(), "handle", &impl,
...
asio::error_code win_iocp_handle_service::cancel(
  ...
  ASIO_HANDLER_OPERATION((iocp_service_.context(), "handle", &impl,
                          reinterpret_cast<uintmax_t>(impl.handle_), "cancel"));
...
size_t win_iocp_handle_service::do_write(
  ...
  ok = ::GetOverlappedResult(impl.handle_, &overlapped, &bytes_transferred,
                             TRUE);
  if (!ok) {
    DWORD last_error = ::GetLastError();
    ec = asio::error_code(last_error, asio::error::get_system_category());
...
void win_iocp_handle_service::start_write_op(
  ...
  if (!is_open(impl)) {
    ...
    BOOL ok =
        ::WriteFile(impl.handle_, buffer.data(),
                    static_cast<DWORD>(buffer.size()), &bytes_transferred, op);
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_read_op(
  ...
  if (!is_open(impl)) {
    ...
    op->OffsetHigh = (offset >> 32) & 0xFFFFFFFF;
    BOOL ok =
        ::ReadFile(impl.handle_, buffer.data(),
                   static_cast<DWORD>(buffer.size()), &bytes_transferred, op);
    DWORD last_error = ::GetLastError();
    if (!ok && last_error != ERROR_IO_PENDING &&
        last_error != ERROR_MORE_DATA) {
...
void *win_iocp_handle_service::interlocked_compare_exchange_pointer(void **dest,
                                                                    void *exch,
                                                                    void *cmp) {
#if defined(_M_IX86)
...
void *win_iocp_handle_service::interlocked_exchange_pointer(void **dest,
                                                            void *val) {
#if defined(_M_IX86)
