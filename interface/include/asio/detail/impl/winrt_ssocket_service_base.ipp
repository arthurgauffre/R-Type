...
winrt_ssocket_service_base::winrt_ssocket_service_base(
    ...
    : scheduler_(use_service<scheduler_impl>(context)),
      async_manager_(use_service<winrt_async_manager>(context)), mutex_(),
      impl_list_(0) {}
...
void winrt_ssocket_service_base::base_move_construct(
    winrt_ssocket_service_base::base_implementation_type &impl,
    winrt_ssocket_service_base::base_implementation_type &other_impl) noexcept {
...
std::size_t winrt_ssocket_service_base::do_get_endpoint(
    const base_implementation_type &impl, bool local, void *addr,
  ...
  try {
    std::string addr_string = winrt_utils::string(
        local ? impl.socket_->Information->LocalAddress->CanonicalName
              : impl.socket_->Information->RemoteAddress->CanonicalName);
    unsigned short port =
        winrt_utils::integer(local ? impl.socket_->Information->LocalPort
    ...
    case ASIO_OS_DEF(AF_INET):
      if (addr_len < sizeof(sockaddr_in4_type)) {
        ...
        socket_ops::inet_pton(
            ASIO_OS_DEF(AF_INET), addr_string.c_str(),
    ...
    case ASIO_OS_DEF(AF_INET6):
      if (addr_len < sizeof(sockaddr_in6_type)) {
        ...
        socket_ops::inet_pton(
            ASIO_OS_DEF(AF_INET6), addr_string.c_str(),
...
asio::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type &impl, int level,
    int optname, const void *optval, std::size_t optlen, asio::error_code &ec) {
  if (!is_open(impl)) {
    ec = asio::error::bad_descriptor;
    return ec;
  }
...
void winrt_ssocket_service_base::do_get_option(
    const winrt_ssocket_service_base::base_implementation_type &impl, int level,
...
asio::error_code winrt_ssocket_service_base::do_connect(
  ...
  if (!ec)
    try {
      async_manager_.sync(
          impl.socket_->ConnectAsync(ref new Windows::Networking::HostName(
                                         winrt_utils::string(addr_string)),
                                     winrt_utils::string(port)),
          ec);
    } catch (Platform::Exception ^ e) {
      ec = asio::error_code(e->HResult, asio::system_category());
    }
...
void winrt_ssocket_service_base::start_connect_op(
  ...
  try {
    ...
    op->ec_ = asio::error_code(e->HResult, asio::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
...
std::size_t winrt_ssocket_service_base::do_send(
  ...
  try {
    buffer_sequence_adapter<asio::const_buffer, asio::const_buffer> bufs(
        asio::buffer(data));
...
std::size_t winrt_ssocket_service_base::do_receive(
  ...
  try {
    buffer_sequence_adapter<asio::mutable_buffer, asio::mutable_buffer> bufs(
        asio::buffer(data));
