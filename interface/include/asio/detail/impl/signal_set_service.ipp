...
void asio_signal_handler(int signal_number) {
  ...
  signed_size_type result =
      ::write(state->write_descriptor_, &signal_number, sizeof(signal_number));
  (void)result;
  errno = saved_errno;
...
public:
  ...
  pipe_read_op()
      : io_uring_operation(asio::error_code(), &pipe_read_op::do_prepare,
                           &pipe_read_op::do_perform,
                           pipe_read_op::do_complete) {}
...
#else  // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
  pipe_read_op()
      : reactor_op(asio::error_code(), &pipe_read_op::do_perform,
                   pipe_read_op::do_complete) {}
...
signal_set_service::~signal_set_service() { remove_service(this); }
...
#if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
      (void)read_descriptor;
      io_uring_service_.register_internal_io_object(
          io_object_data_, io_uring_service::read_op, new pipe_read_op);
#else  // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
      reactor_.register_internal_descriptor(reactor::read_op, read_descriptor,
...
asio::error_code
signal_set_service::add(signal_set_service::implementation_type &impl,
...
asio::error_code
signal_set_service::remove(signal_set_service::implementation_type &impl,
...
asio::error_code
signal_set_service::clear(signal_set_service::implementation_type &impl,
                          asio::error_code &ec) {
  signal_state *state = get_signal_state();
  static_mutex::scoped_lock lock(state->mutex_);
...
asio::error_code
signal_set_service::cancel(signal_set_service::implementation_type &impl,
                           asio::error_code &ec) {
  ASIO_HANDLER_OPERATION(
      (scheduler_.context(), "signal_set", &impl, 0, "cancel"));
...
void signal_set_service::add_service(signal_set_service *service) {
  ...
  if (state->service_list_ != 0) {
    if (!ASIO_CONCURRENCY_HINT_IS_LOCKING(
            SCHEDULER, service->scheduler_.concurrency_hint()) ||
        !ASIO_CONCURRENCY_HINT_IS_LOCKING(
            SCHEDULER, state->service_list_->scheduler_.concurrency_hint())) {
      std::logic_error ex("Thread-unsafe execution context objects require "
...
#else  // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
  service->reactor_.register_internal_descriptor(
      reactor::read_op, read_descriptor, service->reactor_data_,
      new pipe_read_op);
#endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
...
void signal_set_service::remove_service(signal_set_service *service) {
  ...
  if (service->next_ || service->prev_ || state->service_list_ == service) {
    ...
    lock.lock();
#else  // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
    service->reactor_.deregister_internal_descriptor(read_descriptor,
                                                     service->reactor_data_);
