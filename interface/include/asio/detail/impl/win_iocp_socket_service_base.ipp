...
win_iocp_socket_service_base::win_iocp_socket_service_base(
    ...
    : context_(context),
      iocp_service_(use_service<win_iocp_io_context>(context)), reactor_(0),
      connect_ex_(0), nt_set_info_(0), mutex_(), impl_list_(0) {}
...
asio::error_code win_iocp_socket_service_base::close(
  ...
  if (is_open(impl)) {
    ASIO_HANDLER_OPERATION(
        (iocp_service_.context(), "socket", &impl, impl.socket_, "close"));

    // Check if the reactor was created, in which case we need to close the
    // socket on the reactor as well to cancel any operations that might be
    // running there.
    select_reactor *r =
        static_cast<select_reactor *>(interlocked_compare_exchange_pointer(
...
asio::error_code win_iocp_socket_service_base::cancel(
  ...
  ASIO_HANDLER_OPERATION(
      (iocp_service_.context(), "socket", &impl, impl.socket_, "cancel"));

  if (FARPROC cancel_io_ex_ptr =
...
asio::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type &impl, int family,
...
void win_iocp_socket_service_base::start_send_op(
    win_iocp_socket_service_base::base_implementation_type &impl,
    WSABUF *buffers, std::size_t buffer_count, socket_base::message_flags flags,
  ...
  if (noop)
    ...
    int result =
        ::WSASend(impl.socket_, buffers, static_cast<DWORD>(buffer_count),
...
void win_iocp_socket_service_base::start_send_to_op(
    win_iocp_socket_service_base::base_implementation_type &impl,
    WSABUF *buffers, std::size_t buffer_count, const void *addr, int addrlen,
  ...
  if (!is_open(impl))
    ...
    int result = ::WSASendTo(
        ...
        &bytes_transferred, flags, static_cast<const socket_addr_type *>(addr),
        addrlen, op, 0);
    DWORD last_error = ::WSAGetLastError();
    if (last_error == ERROR_PORT_UNREACHABLE)
      last_error = WSAECONNREFUSED;
...
void win_iocp_socket_service_base::start_receive_op(
  ...
  if (noop)
    ...
    int result =
        ::WSARecv(impl.socket_, buffers, static_cast<DWORD>(buffer_count),
...
int win_iocp_socket_service_base::start_null_buffers_receive_op(
  ...
  if ((impl.state_ & socket_ops::stream_oriented) != 0) {
    ...
    int op_type = (flags & socket_base::message_out_of_band)
                      ? select_reactor::except_op
                      : select_reactor::read_op;
...
void win_iocp_socket_service_base::start_receive_from_op(
  ...
  if (!is_open(impl))
    ...
    DWORD recv_flags = flags;
    int result =
        ::WSARecvFrom(impl.socket_, buffers, static_cast<DWORD>(buffer_count),
...
void win_iocp_socket_service_base::restart_accept_op(
  ...
  if (new_socket.get() == invalid_socket)
    ...
    DWORD bytes_read = 0;
    BOOL result = ::AcceptEx(s, new_socket.get(), output_buffer, 0,
                             address_length, address_length, &bytes_read, op);
    DWORD last_error = ::WSAGetLastError();
    if (!result && last_error != WSA_IO_PENDING)
      iocp_service_.on_completion(op, last_error);
...
void win_iocp_socket_service_base::start_reactor_op(
    win_iocp_socket_service_base::base_implementation_type &impl, int op_type,
    reactor_op *op) {
...
int win_iocp_socket_service_base::start_connect_op(
    win_iocp_socket_service_base::base_implementation_type &impl, int family,
    int type, const void *addr, std::size_t addrlen,
    win_iocp_socket_connect_op_base *op, operation *iocp_op) {
  // If ConnectEx is available, use that.
  if (family == ASIO_OS_DEF(AF_INET) || family == ASIO_OS_DEF(AF_INET6)) {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type)) {
      ...
      socket_ops::bind(impl.socket_, &a.base,
                       family == ASIO_OS_DEF(AF_INET) ? sizeof(a.v4)
                                                      : sizeof(a.v6),
                       op->ec_);
      ...
      BOOL result =
          connect_ex(impl.socket_, static_cast<const socket_addr_type *>(addr),
  ...
  if ((impl.state_ & socket_ops::non_blocking) != 0 ||
      socket_ops::set_internal_non_blocking(impl.socket_, impl.state_, true,
                                            op->ec_)) {
    if (socket_ops::connect(impl.socket_, addr, addrlen, op->ec_) != 0) {
      if (op->ec_ == asio::error::in_progress ||
          op->ec_ == asio::error::would_block) {
        op->ec_ = asio::error_code();
        r.start_op(select_reactor::connect_op, impl.socket_, impl.reactor_data_,
                   op, false, false);
...
win_iocp_socket_service_base::get_connect_ex(
  ...
  if (type != ASIO_OS_DEF(SOCK_STREAM) && type != ASIO_OS_DEF(SOCK_SEQPACKET))
    return 0;

  void *ptr = interlocked_compare_exchange_pointer(&connect_ex_, 0, 0);
  if (!ptr) {
    GUID guid = {0x25a207b9,
                 0xddf3,
                 0x4660,
                 {0x8e, 0xe9, 0x76, 0xe5, 0x8c, 0x74, 0x06, 0x3e}};

    DWORD bytes = 0;
    if (::WSAIoctl(impl.socket_, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid,
                   sizeof(guid), &ptr, sizeof(ptr), &bytes, 0, 0) != 0) {
...
void *win_iocp_socket_service_base::interlocked_exchange_pointer(void **dest,
                                                                 void *val) {
#if defined(_M_IX86)
