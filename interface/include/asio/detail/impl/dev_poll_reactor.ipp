...
dev_poll_reactor::dev_poll_reactor(asio::execution_context &ctx)
    : asio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
      scheduler_(use_service<scheduler>(ctx)), mutex_(),
      dev_poll_fd_(do_dev_poll_create()), interrupter_(), shutdown_(false) {
  ...
  if (fork_ev == asio::execution_context::fork_child) {
    ...
    for (int i = 0; i < max_ops; ++i) {
      ...
      for (; iter != end; ++iter) {
        ...
        switch (i) {
        case read_op:
          pending_ev.events |= POLLIN;
          break;
        case write_op:
          pending_ev.events |= POLLOUT;
          break;
        case except_op:
          pending_ev.events |= POLLPRI;
          break;
        default:
          break;
        }
...
void dev_poll_reactor::init_task() { scheduler_.init_task(); }
...
void dev_poll_reactor::move_descriptor(
    socket_type, dev_poll_reactor::per_descriptor_data &,
    dev_poll_reactor::per_descriptor_data &) {}

void dev_poll_reactor::call_post_immediate_completion(operation *op,
...
void dev_poll_reactor::start_op(int op_type, socket_type descriptor,
  ...
  if (first) {
    ::pollfd &ev = add_pending_event_change(descriptor);
    ev.events = POLLERR | POLLHUP;
    if (op_type == read_op || op_queue_[read_op].has_operation(descriptor))
      ev.events |= POLLIN;
...
void dev_poll_reactor::cancel_ops_by_key(
...
void dev_poll_reactor::deregister_descriptor(
    socket_type descriptor, dev_poll_reactor::per_descriptor_data &, bool) {
  asio::detail::mutex::scoped_lock lock(mutex_);
...
void dev_poll_reactor::run(long usec, op_queue<operation> &ops) {
  ...
  if (events_size > 0) {
    errno = 0;
    int result = ::write(dev_poll_fd_, &pending_event_changes_[0], events_size);
  ...
  for (int i = 0; i < num_events; ++i) {
    ...
    if (descriptor == interrupter_.read_descriptor()) {
      ...
      if (events[i].events & (POLLPRI | POLLERR | POLLHUP))
        more_except = op_queue_[except_op].perform_operations(descriptor, ops);
      else
      ...
      if ((events[i].events & (POLLERR | POLLHUP)) != 0 && !more_except &&
          !more_reads && !more_writes) {
...
void dev_poll_reactor::interrupt() { interrupter_.interrupt(); }
