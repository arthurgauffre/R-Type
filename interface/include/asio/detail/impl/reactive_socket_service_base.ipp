...
void reactive_socket_service_base::base_move_construct(
  ...
  reactor_.move_descriptor(impl.socket_, impl.reactor_data_,
...
void reactive_socket_service_base::base_move_assign(
  ...
  other_service.reactor_.move_descriptor(impl.socket_, impl.reactor_data_,
                                         other_impl.reactor_data_);
}
...
void reactive_socket_service_base::destroy(
  ...
  if (impl.socket_ != invalid_socket) {
    ASIO_HANDLER_OPERATION(
        (reactor_.context(), "socket", &impl, impl.socket_, "close"));
...
socket_type reactive_socket_service_base::release(
  ...
  ASIO_HANDLER_OPERATION(
      (reactor_.context(), "socket", &impl, impl.socket_, "release"));
...
asio::error_code reactive_socket_service_base::cancel(
  ...
  ASIO_HANDLER_OPERATION(
      (reactor_.context(), "socket", &impl, impl.socket_, "cancel"));
...
asio::error_code reactive_socket_service_base::do_open(
    reactive_socket_service_base::base_implementation_type &impl, int af,
...
void reactive_socket_service_base::do_start_accept_op(
    ...
    const void *immediate_arg) {
  if (!peer_is_open) {
    do_start_op(impl, reactor::read_op, op, is_continuation, true, false,
                on_immediate, immediate_arg);
  } else {
    ...
    if (socket_ops::connect(impl.socket_, addr, addrlen, op->ec_) != 0) {
      if (op->ec_ == asio::error::in_progress ||
        ...
        reactor_.start_op(reactor::connect_op, impl.socket_, impl.reactor_data_,
                          op, is_continuation, false, on_immediate,
                          immediate_arg);
        return;
      }
    }
