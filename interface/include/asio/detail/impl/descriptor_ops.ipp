...
int open(const char *path, int flags, unsigned mode, asio::error_code &ec) {
...
bool set_user_non_blocking(int d, state_type &state, bool value,
                           asio::error_code &ec) {
  if (d == -1) {
...
bool set_internal_non_blocking(int d, state_type &state, bool value,
                               asio::error_code &ec) {
  if (d == -1) {
    ec = asio::error::bad_descriptor;
    return false;
...
std::size_t sync_read(int d, state_type state, buf *bufs, std::size_t count,
...
std::size_t sync_read1(int d, state_type state, void *data, std::size_t size,
                       asio::error_code &ec) {
  if (d == -1) {
    ec = asio::error::bad_descriptor;
    return 0;
...
std::size_t sync_read_at(int d, state_type state, uint64_t offset, buf *bufs,
                         std::size_t count, bool all_empty,
                         asio::error_code &ec) {
  if (d == -1) {
    ec = asio::error::bad_descriptor;
...
std::size_t sync_read_at1(int d, state_type state, uint64_t offset, void *data,
                          std::size_t size, asio::error_code &ec) {
  if (d == -1) {
    ec = asio::error::bad_descriptor;
    return 0;
...
std::size_t sync_write_at(int d, state_type state, uint64_t offset,
  ...
  for (;;) {
    ...
    signed_size_type bytes =
        ::pwritev(d, bufs, static_cast<int>(count), offset);
    get_last_error(ec, bytes < 0);
...
bool non_blocking_write_at(int d, uint64_t offset, const buf *bufs,
...
bool non_blocking_write_at1(int d, uint64_t offset, const void *data,
                            std::size_t size, asio::error_code &ec,
                            std::size_t &bytes_transferred) {
  for (;;) {
    // Write some data.
...
int ioctl(int d, state_type &state, long cmd, ioctl_arg_type *arg,
          asio::error_code &ec) {
  if (d == -1) {
    ec = asio::error::bad_descriptor;
    return -1;
