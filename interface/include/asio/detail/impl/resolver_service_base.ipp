...
resolver_service_base::resolver_service_base(execution_context &context)
    : scheduler_(asio::use_service<scheduler_impl>(context)),
      work_scheduler_(new scheduler_impl(context, -1, false)), work_thread_(0) {
  work_scheduler_->work_started();
}

resolver_service_base::~resolver_service_base() { base_shutdown(); }
...
void resolver_service_base::destroy(
  ...
  ASIO_HANDLER_OPERATION(
      (scheduler_.context(), "resolver", &impl, 0, "cancel"));
...
void resolver_service_base::start_work_thread() {
  ...
  if (!work_thread_.get()) {
    work_thread_.reset(
        new asio::detail::thread(work_scheduler_runner(*work_scheduler_)));
  }
}
