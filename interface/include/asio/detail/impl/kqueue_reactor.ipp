...
kqueue_reactor::kqueue_reactor(asio::execution_context &ctx)
    : execution_context_service_base<kqueue_reactor>(ctx),
      ...
      mutex_(ASIO_CONCURRENCY_HINT_IS_LOCKING(REACTOR_REGISTRATION,
                                              scheduler_.concurrency_hint())),
      kqueue_fd_(do_kqueue_create()), interrupter_(), shutdown_(false),
      registered_descriptors_mutex_(mutex_.enabled()) {
  struct kevent events[1];
  ASIO_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(), EVFILT_READ,
                     EV_ADD, 0, 0, &interrupter_);
...
kqueue_reactor::~kqueue_reactor() { close(kqueue_fd_); }
...
void kqueue_reactor::notify_fork(asio::execution_context::fork_event fork_ev) {
  if (fork_ev == asio::execution_context::fork_child) {
    ...
    for (descriptor_state *state = registered_descriptors_.first(); state != 0;
         state = state->next_) {
      if (state->num_kevents_ > 0) {
        ASIO_KQUEUE_EV_SET(&events[0], state->descriptor_, EVFILT_READ,
                           EV_ADD | EV_CLEAR, 0, 0, state);
        ASIO_KQUEUE_EV_SET(&events[1], state->descriptor_, EVFILT_WRITE,
                           EV_ADD | EV_CLEAR, 0, 0, state);
...
void kqueue_reactor::init_task() { scheduler_.init_task(); }

int kqueue_reactor::register_descriptor(
...
int kqueue_reactor::register_internal_descriptor(
  ...
  struct kevent events[1];
  ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ, EV_ADD | EV_CLEAR, 0,
                     0, descriptor_data);
  if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    return errno;
...
void kqueue_reactor::move_descriptor(
    socket_type, kqueue_reactor::per_descriptor_data &target_descriptor_data,
    kqueue_reactor::per_descriptor_data &source_descriptor_data) {
  target_descriptor_data = source_descriptor_data;
  source_descriptor_data = 0;
}

void kqueue_reactor::call_post_immediate_completion(operation *op,
    ...
    if (allow_speculative &&
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type]) {
        ...
        if (::kevent(kqueue_fd_, events, num_kevents[op_type], 0, 0, 0) != -1) {
          ...
          op->ec_ = asio::error_code(errno, asio::error::get_system_category());
          on_immediate(op, is_continuation, immediate_arg);
          return;
        }
      }
      ...
      struct kevent events[2];
      ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ, EV_ADD | EV_CLEAR,
                         0, 0, descriptor_data);
      ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
                         EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
...
void kqueue_reactor::cancel_ops(
    socket_type, kqueue_reactor::per_descriptor_data &descriptor_data) {
  if (!descriptor_data)
    return;
...
void kqueue_reactor::cancel_ops_by_key(
    socket_type, kqueue_reactor::per_descriptor_data &descriptor_data,
...
void kqueue_reactor::deregister_descriptor(
    socket_type descriptor,
  ...
  if (!descriptor_data->shutdown_) {
    if (closing) {
      ...
      struct kevent events[2];
      ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ, EV_DELETE, 0, 0,
                         0);
      ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE, EV_DELETE, 0, 0,
                         0);
      ::kevent(kqueue_fd_, events, descriptor_data->num_kevents_, 0, 0, 0);
    }
...
void kqueue_reactor::deregister_internal_descriptor(
    socket_type descriptor,
    kqueue_reactor::per_descriptor_data &descriptor_data) {
  if (!descriptor_data)
    return;
  ...
  if (!descriptor_data->shutdown_) {
    struct kevent events[2];
    ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ, EV_DELETE, 0, 0, 0);
    ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE, EV_DELETE, 0, 0,
                       0);
    ::kevent(kqueue_fd_, events, descriptor_data->num_kevents_, 0, 0, 0);
...
void kqueue_reactor::run(long usec, op_queue<operation> &ops) {
  ...
  for (int i = 0; i < num_events; ++i) {
    ...
    if (ptr != &interrupter_) {
      ...
      ASIO_HANDLER_REACTOR_EVENTS(
          (context(), reinterpret_cast<uintmax_t>(ptr), event_mask));
    }
  }
  ...
  for (int i = 0; i < num_events; ++i) {
    ...
    if (ptr == &interrupter_) {
      ...
      if (events[i].filter == EVFILT_WRITE &&
        ...
        ASIO_KQUEUE_EV_SET(&delete_events[0], descriptor_data->descriptor_,
          ...
          if (j != except_op || events[i].flags & EV_OOBAND) {
            while (reactor_op *op = descriptor_data->op_queue_[j].front()) {
              if (events[i].flags & EV_ERROR) {
                op->ec_ = asio::error_code(static_cast<int>(events[i].data),
...
void kqueue_reactor::interrupt() { interrupter_.interrupt(); }
...
void kqueue_reactor::free_descriptor_state(
    kqueue_reactor::descriptor_state *s) {
  mutex::scoped_lock descriptors_lock(registered_descriptors_mutex_);
