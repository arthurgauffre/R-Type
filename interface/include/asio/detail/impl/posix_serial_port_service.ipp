...
posix_serial_port_service::posix_serial_port_service(execution_context &context)
    : execution_context_service_base<posix_serial_port_service>(context),
      descriptor_service_(context) {}

void posix_serial_port_service::shutdown() { descriptor_service_.shutdown(); }
...
asio::error_code posix_serial_port_service::open(
  ...
  int fd =
      descriptor_ops::open(device.c_str(), O_RDWR | O_NONBLOCK | O_NOCTTY, ec);
  if (fd < 0) {
    ASIO_ERROR_LOCATION(ec);
    return ec;
  }
...
asio::error_code posix_serial_port_service::do_set_option(
    posix_serial_port_service::implementation_type &impl,
    posix_serial_port_service::store_function_type store, const void *option,
    asio::error_code &ec) {
  termios ios;
...
asio::error_code posix_serial_port_service::do_get_option(
    ...
    posix_serial_port_service::load_function_type load, void *option,
