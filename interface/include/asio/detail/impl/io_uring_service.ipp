...
io_uring_service::io_uring_service(asio::execution_context &ctx)
    : execution_context_service_base<io_uring_service>(ctx),
      ...
      outstanding_work_(0), submit_sqes_op_(this), pending_sqes_(0),
      pending_submit_sqes_op_(false), shutdown_(false), timeout_(),
      registration_mutex_(mutex_.enabled()),
      reactor_(use_service<reactor>(ctx)), reactor_data_(), event_fd_(-1) {
  reactor_.init_task();
  init_ring();
...
void io_uring_service::notify_fork(
  ...
  case asio::execution_context::fork_prepare: {
    ...
    for (io_object *io_obj = registered_io_objects_.first(); io_obj != 0;
         io_obj = io_obj->next_) {
...
void io_uring_service::init_task() { scheduler_.init_task(); }
...
void io_uring_service::cancel_ops(
    io_uring_service::per_io_object_data &io_obj) {
  if (!io_obj)
    return;
...
void io_uring_service::run(long usec, op_queue<operation> &ops) {
  ...
  ::io_uring_cqe *cqe = 0;
  int result = (usec == 0) ? ::io_uring_peek_cqe(&ring_, &cqe)
...
public:
  event_fd_read_op(io_uring_service *s)
      : reactor_op(asio::error_code(), &event_fd_read_op::do_perform,
  ...
  static status do_perform(reactor_op *base) {
    ...
    for (;;) {
      ...
      errno = 0;
      int bytes_read =
          ::read(o->service_->event_fd_, &counter, sizeof(uint64_t));
...
void io_uring_service::register_with_reactor() {
  ...
  reactor_.register_internal_descriptor(
      reactor::read_op, event_fd_, reactor_data_, new event_fd_read_op(this));
#endif // !defined(ASIO_HAS_IO_URING_AS_DEFAULT)
}

io_uring_service::io_object *io_uring_service::allocate_io_object() {
  mutex::scoped_lock registration_lock(registration_mutex_);
  return registered_io_objects_.alloc(ASIO_CONCURRENCY_HINT_IS_LOCKING(
...
bool io_uring_service::do_cancel_ops(per_io_object_data &io_obj,
                                     op_queue<operation> &ops) {
...
io_uring_service::submit_sqes_op::submit_sqes_op(io_uring_service *s)
    : operation(&io_uring_service::submit_sqes_op::do_complete), service_(s) {}
...
io_uring_service::io_object::io_object(bool locking) : mutex_(locking) {}
