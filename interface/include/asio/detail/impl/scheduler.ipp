...
public:
  explicit thread_function(scheduler *s) : this_(s) {}
...
struct scheduler::task_cleanup {
  ~task_cleanup() {
    if (this_thread_->private_outstanding_work > 0) {
      asio::detail::increment(scheduler_->outstanding_work_,
...
scheduler::scheduler(asio::execution_context &ctx, int concurrency_hint,
    ...
    : asio::detail::execution_context_service_base<scheduler>(ctx),
      ...
      task_(0), get_task_(get_task), task_interrupted_(true),
      outstanding_work_(0), stopped_(false), shutdown_(false),
      concurrency_hint_(concurrency_hint), thread_(0) {
...
void scheduler::post_immediate_completion(scheduler::operation *op,
...
void scheduler::post_deferred_completions(op_queue<scheduler::operation> &ops) {
  if (!ops.empty()) {
#if defined(ASIO_HAS_THREADS)
    if (one_thread_) {
      if (thread_info_base *this_thread = thread_call_stack::contains(this)) {
...
void scheduler::abandon_operations(op_queue<scheduler::operation> &ops) {
