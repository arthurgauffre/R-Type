...
inline socket_type call_accept(SockLenType msghdr::*, socket_type s, void *addr,
                               std::size_t *addrlen) {
  SockLenType tmp_addrlen = addrlen ? (SockLenType)*addrlen : 0;
  socket_type result = ::accept(s, static_cast<socket_addr_type *>(addr),
...
socket_type accept(socket_type s, void *addr, std::size_t *addrlen,
                   asio::error_code &ec) {
  if (s == invalid_socket) {
  ...
  int optval = 1;
  int result =
      ::setsockopt(new_s, SOL_SOCKET, SO_NOSIGPIPE, &optval, sizeof(optval));
...
socket_type sync_accept(socket_type s, state_type state, void *addr,
...
void complete_iocp_accept(socket_type s, void *output_buffer,
                          DWORD address_length, void *addr,
                          std::size_t *addrlen, socket_type new_socket,
                          asio::error_code &ec) {
  ...
  if (!ec) {
    ...
    if (addr && addrlen) {
      ...
      GetAcceptExSockaddrs(output_buffer, 0, address_length, address_length,
                           &local_addr, &local_addr_length, &remote_addr,
                           &remote_addr_length);
    ...
    socket_ops::setsockopt(new_socket, state, SOL_SOCKET,
                           SO_UPDATE_ACCEPT_CONTEXT, &update_ctx_param,
                           sizeof(SOCKET), ec);
  }
}
...
bool non_blocking_accept(socket_type s, state_type state, void *addr,
                         std::size_t *addrlen, asio::error_code &ec,
...
template <typename SockLenType>
inline int call_bind(SockLenType msghdr::*, socket_type s, const void *addr,
                     std::size_t addrlen) {
  return ::bind(s, static_cast<const socket_addr_type *>(addr),
                (SockLenType)addrlen);
}

int bind(socket_type s, const void *addr, std::size_t addrlen,
         asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
...
int close(socket_type s, state_type &state, bool destruction,
          asio::error_code &ec) {
  int result = 0;
  if (s != invalid_socket) {
    ...
    if (destruction && (state & user_set_linger)) {
      ...
      socket_ops::setsockopt(s, state, SOL_SOCKET, SO_LINGER, &opt, sizeof(opt),
                             ignored_ec);
    }
...
bool set_user_non_blocking(socket_type s, state_type &state, bool value,
                           asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return false;
...
bool set_internal_non_blocking(socket_type s, state_type &state, bool value,
                               asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return false;
...
template <typename SockLenType>
inline int call_connect(SockLenType msghdr::*, socket_type s, const void *addr,
                        std::size_t addrlen) {
  return ::connect(s, static_cast<const socket_addr_type *>(addr),
                   (SockLenType)addrlen);
}

int connect(socket_type s, const void *addr, std::size_t addrlen,
            asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
...
void sync_connect(socket_type s, const void *addr, std::size_t addrlen,
                  asio::error_code &ec) {
  ...
  if (socket_ops::getsockopt(s, 0, SOL_SOCKET, SO_ERROR, &connect_error,
                             &connect_error_len, ec) == socket_error_retval)
    return;

  // Return the result of the connect operation.
  ec = asio::error_code(connect_error, asio::error::get_system_category());
}
...
void complete_iocp_connect(socket_type s, asio::error_code &ec) {
  ...
  if (!ec) {
    ...
    socket_ops::setsockopt(s, state, SOL_SOCKET, so_update_connect_context, 0,
                           0, ec);
  }
}
...
int socketpair(int af, int type, int protocol, socket_type sv[2],
               asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
...
template <typename T> inline void init_buf_iov_base(T &base, void *addr) {
...
template <typename T> inline void init_msghdr_msg_name(T &name, void *addr) {
...
signed_size_type recv(socket_type s, buf *bufs, size_t count, int flags,
  ...
  int result =
      ::WSARecv(s, bufs, recv_buf_count, &bytes_transferred, &recv_flags, 0, 0);
...
signed_size_type recv1(socket_type s, void *data, size_t size, int flags,
                       asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  WSABUF buf;
  ...
  DWORD bytes_transferred = 0;
  DWORD recv_flags = flags;
  int result = ::WSARecv(s, &buf, 1, &bytes_transferred, &recv_flags, 0, 0);
  get_last_error(ec, true);
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = asio::error::connection_reset;
  else if (ec.value() == ERROR_PORT_UNREACHABLE)
...
size_t sync_recv(socket_type s, state_type state, buf *bufs, size_t count,
...
size_t sync_recv1(socket_type s, state_type state, void *data, size_t size,
...
void complete_iocp_recv(state_type state,
  ...
  else if (!ec && bytes_transferred == 0 && (state & stream_oriented) != 0 &&
           !all_empty) {
    ec = asio::error::eof;
  }
}
...
bool non_blocking_recv(socket_type s, buf *bufs, size_t count, int flags,
                       bool is_stream, asio::error_code &ec,
                       size_t &bytes_transferred) {
  for (;;) {
    // Read some data.
    signed_size_type bytes = socket_ops::recv(s, bufs, count, flags, ec);
...
bool non_blocking_recv1(socket_type s, void *data, size_t size, int flags,
                        bool is_stream, asio::error_code &ec,
                        size_t &bytes_transferred) {
  for (;;) {
    // Read some data.
...
signed_size_type recvfrom(socket_type s, buf *bufs, size_t count, int flags,
                          void *addr, std::size_t *addrlen,
                          asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  DWORD recv_buf_count = static_cast<DWORD>(count);
  ...
  int result =
      ::WSARecvFrom(s, bufs, recv_buf_count, &bytes_transferred, &recv_flags,
...
inline signed_size_type call_recvfrom(SockLenType msghdr::*, socket_type s,
                                      ...
                                      void *addr, std::size_t *addrlen) {
  SockLenType tmp_addrlen = addrlen ? (SockLenType)*addrlen : 0;
  signed_size_type result = ::recvfrom(
      s, static_cast<char *>(data), size, flags,
      static_cast<socket_addr_type *>(addr), addrlen ? &tmp_addrlen : 0);
  if (addrlen)
    *addrlen = (std::size_t)tmp_addrlen;
  return result;
}

signed_size_type recvfrom1(socket_type s, void *data, size_t size, int flags,
                           void *addr, std::size_t *addrlen,
                           asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  ...
  int tmp_addrlen = (int)*addrlen;
  int result =
      ::WSARecvFrom(s, &buf, 1, &bytes_transferred, &recv_flags,
                    static_cast<socket_addr_type *>(addr), &tmp_addrlen, 0, 0);
  get_last_error(ec, true);
  *addrlen = (std::size_t)tmp_addrlen;
  if (ec.value() == ERROR_NETNAME_DELETED)
  ...
  signed_size_type result =
      call_recvfrom(&msghdr::msg_namelen, s, data, size, flags, addr, addrlen);
  get_last_error(ec, result < 0);
  return result;
#endif // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
}
...
size_t sync_recvfrom(socket_type s, state_type state, buf *bufs, size_t count,
  ...
  for (;;) {
    // Try to complete the operation without blocking.
    signed_size_type bytes =
        socket_ops::recvfrom(s, bufs, count, flags, addr, addrlen, ec);
...
size_t sync_recvfrom1(socket_type s, state_type state, void *data, size_t size,
  ...
  for (;;) {
    // Try to complete the operation without blocking.
    signed_size_type bytes =
        socket_ops::recvfrom1(s, data, size, flags, addr, addrlen, ec);
...
void complete_iocp_recvfrom(const weak_cancel_token_type &cancel_token,
                            asio::error_code &ec) {
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED) {
    if (cancel_token.expired())
...
bool non_blocking_recvfrom1(socket_type s, void *data, size_t size, int flags,
                            void *addr, std::size_t *addrlen,
                            asio::error_code &ec, size_t &bytes_transferred) {
  for (;;) {
    // Read some data.
...
signed_size_type recvmsg(socket_type s, buf *bufs, size_t count, int in_flags,
...
size_t sync_recvmsg(socket_type s, state_type state, buf *bufs, size_t count,
  ...
  for (;;) {
    // Try to complete the operation without blocking.
    signed_size_type bytes =
        socket_ops::recvmsg(s, bufs, count, in_flags, out_flags, ec);
...
void complete_iocp_recvmsg(const weak_cancel_token_type &cancel_token,
                           asio::error_code &ec) {
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED) {
    if (cancel_token.expired())
...
bool non_blocking_recvmsg(socket_type s, buf *bufs, size_t count, int in_flags,
                          int &out_flags, asio::error_code &ec,
                          size_t &bytes_transferred) {
  for (;;) {
    // Read some data.
...
signed_size_type send(socket_type s, const buf *bufs, size_t count, int flags,
                      asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Send the data.
  ...
  int result = ::WSASend(s, const_cast<buf *>(bufs), send_buf_count,
...
signed_size_type send1(socket_type s, const void *data, size_t size, int flags,
                       asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Send the data.
  WSABUF buf;
  ...
  DWORD bytes_transferred = 0;
  DWORD send_flags = flags;
  int result = ::WSASend(s, &buf, 1, &bytes_transferred, send_flags, 0, 0);
  get_last_error(ec, true);
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = asio::error::connection_reset;
  else if (ec.value() == ERROR_PORT_UNREACHABLE)
...
#endif // defined(ASIO_HAS_MSG_NOSIGNAL)
  signed_size_type result =
      ::send(s, static_cast<const char *>(data), size, flags);
  get_last_error(ec, result < 0);
  return result;
#endif // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
}

size_t sync_send(socket_type s, state_type state, const buf *bufs, size_t count,
                 int flags, bool all_empty, asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return 0;
...
void complete_iocp_send(const weak_cancel_token_type &cancel_token,
                        asio::error_code &ec) {
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED) {
    if (cancel_token.expired())
...
bool non_blocking_send(socket_type s, const buf *bufs, size_t count, int flags,
                       asio::error_code &ec, size_t &bytes_transferred) {
  for (;;) {
    // Write some data.
    signed_size_type bytes = socket_ops::send(s, bufs, count, flags, ec);
...
bool non_blocking_send1(socket_type s, const void *data, size_t size, int flags,
                        asio::error_code &ec, size_t &bytes_transferred) {
  for (;;) {
    // Write some data.
    signed_size_type bytes = socket_ops::send1(s, data, size, flags, ec);
...
signed_size_type sendto(socket_type s, const buf *bufs, size_t count, int flags,
...
template <typename SockLenType>
inline signed_size_type call_sendto(SockLenType msghdr::*, socket_type s,
...
signed_size_type sendto1(socket_type s, const void *data, size_t size,
                         int flags, const void *addr, std::size_t addrlen,
                         asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Send the data.
  WSABUF buf;
...
#endif // defined(ASIO_HAS_MSG_NOSIGNAL)
  signed_size_type result =
      call_sendto(&msghdr::msg_namelen, s, data, size, flags, addr, addrlen);
  get_last_error(ec, result < 0);
  return result;
#endif // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
}
...
size_t sync_sendto(socket_type s, state_type state, const buf *bufs,
  ...
  for (;;) {
    // Try to complete the operation without blocking.
    signed_size_type bytes =
        socket_ops::sendto(s, bufs, count, flags, addr, addrlen, ec);
...
size_t sync_sendto1(socket_type s, state_type state, const void *data,
  ...
  for (;;) {
    // Try to complete the operation without blocking.
    signed_size_type bytes =
        socket_ops::sendto1(s, data, size, flags, addr, addrlen, ec);
...
bool non_blocking_sendto(socket_type s, const buf *bufs, size_t count,
                         int flags, const void *addr, std::size_t addrlen,
                         asio::error_code &ec, size_t &bytes_transferred) {
  for (;;) {
    // Write some data.
...
bool non_blocking_sendto1(socket_type s, const void *data, size_t size,
                          int flags, const void *addr, std::size_t addrlen,
                          asio::error_code &ec, size_t &bytes_transferred) {
  for (;;) {
    // Write some data.
...
socket_type socket(int af, int type, int protocol, asio::error_code &ec) {
  ...
  int optval = 1;
  int result =
      ::setsockopt(s, SOL_SOCKET, SO_NOSIGPIPE, &optval, sizeof(optval));
  get_last_error(ec, result != 0);
  if (result != 0) {
    ::close(s);
    return invalid_socket;
...
template <typename SockLenType>
inline int call_setsockopt(SockLenType msghdr::*, socket_type s, int level,
                           int optname, const void *optval,
                           std::size_t optlen) {
  return ::setsockopt(s, level, optname, (const char *)optval,
                      (SockLenType)optlen);
}
...
int setsockopt(socket_type s, state_type &state, int level, int optname,
  ...
  if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
    ...
    if (sso_t sso = (sso_t)::GetProcAddress(winsock_module, "setsockopt")) {
      int result =
          sso(s, level, optname, reinterpret_cast<const char *>(optval),
  ...
  int result =
      call_setsockopt(&msghdr::msg_namelen, s, level, optname, optval, optlen);
  get_last_error(ec, result != 0);
  if (result == 0) {
    ...
    if ((state & datagram_oriented) && level == SOL_SOCKET &&
        optname == SO_REUSEADDR) {
      call_setsockopt(&msghdr::msg_namelen, s, SOL_SOCKET, SO_REUSEPORT, optval,
                      optlen);
    }
#endif
  }
...
template <typename SockLenType>
inline int call_getsockopt(SockLenType msghdr::*, socket_type s, int level,
                           int optname, void *optval, std::size_t *optlen) {
...
int getsockopt(socket_type s, state_type state, int level, int optname,
  ...
  if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
    ...
    if (gso_t gso = (gso_t)::GetProcAddress(winsock_module, "getsockopt")) {
      ...
      int result =
          gso(s, level, optname, reinterpret_cast<char *>(optval), &tmp_optlen);
      get_last_error(ec, result != 0);
  ...
  int result =
      call_getsockopt(&msghdr::msg_namelen, s, level, optname, optval, optlen);
  get_last_error(ec, result != 0);
  if (result != 0 && level == IPPROTO_IPV6 && optname == IPV6_V6ONLY &&
      ec.value() == WSAENOPROTOOPT && *optlen == sizeof(DWORD)) {
...
template <typename SockLenType>
inline int call_getpeername(SockLenType msghdr::*, socket_type s, void *addr,
                            std::size_t *addrlen) {
  SockLenType tmp_addrlen = (SockLenType)*addrlen;
  int result =
      ::getpeername(s, static_cast<socket_addr_type *>(addr), &tmp_addrlen);
  *addrlen = (std::size_t)tmp_addrlen;
  return result;
}

int getpeername(socket_type s, void *addr, std::size_t *addrlen, bool cached,
                asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
...
#if defined(ASIO_WINDOWS) && !defined(ASIO_WINDOWS_APP) || defined(__CYGWIN__)
  if (cached) {
    // Check if socket is still connected.
    DWORD connect_time = 0;
    size_t connect_time_len = sizeof(connect_time);
    if (socket_ops::getsockopt(s, 0, SOL_SOCKET, SO_CONNECT_TIME, &connect_time,
...
template <typename SockLenType>
inline int call_getsockname(SockLenType msghdr::*, socket_type s, void *addr,
                            std::size_t *addrlen) {
  SockLenType tmp_addrlen = (SockLenType)*addrlen;
  int result =
      ::getsockname(s, static_cast<socket_addr_type *>(addr), &tmp_addrlen);
  *addrlen = (std::size_t)tmp_addrlen;
  return result;
}

int getsockname(socket_type s, void *addr, std::size_t *addrlen,
                asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
...
int ioctl(socket_type s, state_type &state, int cmd, ioctl_arg_type *arg,
          asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
...
int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
  ...
  int result =
      ::pselect(nfds, readfds, writefds, exceptfds, timeout ? &ts : 0, 0);
#else
...
int poll_read(socket_type s, state_type state, int msec, asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
  }
...
int poll_write(socket_type s, state_type state, int msec,
               asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
...
int poll_error(socket_type s, state_type state, int msec,
               asio::error_code &ec) {
  if (s == invalid_socket) {
    ec = asio::error::bad_descriptor;
    return socket_error_retval;
...
const char *inet_ntop(int af, const void *src, char *dest, size_t length,
  ...
  if (af == ASIO_OS_DEF(AF_INET)) {
    sprintf_s(dest, length, "%u.%u.%u.%u", bytes[0], bytes[1], bytes[2],
              bytes[3]);
    return dest;
    ...
    while (n < length && b < 16) {
      if (bytes[b] == 0 && bytes[b + 1] == 0 && z == 0) {
        ...
        n +=
            sprintf_s(dest + n, length - n, "%s%x", b ? ":" : "",
  ...
  int result = ::WSAAddressToStringW(&address.base, address_length, 0,
                                     string_buffer, &string_length);
  get_last_error(ec, true);
  ::WideCharToMultiByte(CP_ACP, 0, string_buffer, -1, dest,
                        static_cast<int>(length), 0, 0);
#else
  int result = ::WSAAddressToStringA(&address.base, address_length, 0, dest,
                                     &string_length);
  get_last_error(ec, true);
#endif
...
int inet_pton(int af, const char *src, void *dest, unsigned long *scope_id,
  ...
  int result =
      ::WSAStringToAddressW(wide_buffer, af, 0, &address.base, &address_length);
  get_last_error(ec, true);
#else
  ...
  try {
    ...
    for (unsigned i = 0; i < hostnames->Size; ++i) {
      ...
      if (hostname->Type == HostNameType::DomainName) {
        ...
        if (namelen > 0 &&
            raw_name.size() < static_cast<std::size_t>(namelen)) {
...
inline hostent *gethostbyaddr(const char *addr, int length, int af,
                              hostent *result, char *buffer, int buflength,
                              asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
  ...
  *result = *retval;
  ...
  int error = 0;
  hostent *retval =
      ::gethostbyaddr_r(addr, length, af, result, buffer, buflength, &error);
  get_last_error(ec, !retval);
  if (error)
  ...
  *result = *retval;
  ...
  int error = 0;
  clear_last_error();
  ::gethostbyaddr_r(addr, length, af, result, buffer, buflength, &retval,
                    &error);
  get_last_error(ec, true);
  if (error)
    ec = translate_netdb_error(error);
...
inline hostent *gethostbyname(const char *name, int af, struct hostent *result,
                              char *buffer, int buflength, int ai_flags,
                              asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
...
inline int gai_nsearch(const char *host, const addrinfo_type *hints,
                       gai_search (&search)[2]) {
  int search_count = 0;
  if (host == 0 || host[0] == '\0') {
...
template <typename T> inline T *gai_alloc(std::size_t size = sizeof(T)) {
  using namespace std;
...
inline void gai_free(void *p) { ::operator delete(p); }
...
inline int gai_serv(addrinfo_type *aihead, const addrinfo_type *hints,
                    const char *serv) {
  using namespace std;
...
inline int gai_echeck(const char *host, const char *service, int flags,
...
inline int getaddrinfo_emulation(const char *host, const char *service,
  ...
  if (hintsp)
    ...
    // If the resolution is not specifically for AF_INET6, remove the
    // AI_V4MAPPED and AI_ALL flags.
  ...
  for (gai_search *sptr = search; sptr < search + search_count; ++sptr) {
    ...
    asio::error_code ec;
    if (socket_ops::inet_pton(ASIO_OS_DEF(AF_INET), sptr->host, &inaddr, 0,
                              ec) == 1) {
    ...
    if (socket_ops::inet_pton(ASIO_OS_DEF(AF_INET6), sptr->host, &in6addr, 0,
                              ec) == 1) {
      if (hints.ai_family != ASIO_OS_DEF(AF_UNSPEC) &&
      ...
      if (sptr->family == ASIO_OS_DEF(AF_INET6)) {
        rc = gai_aistruct(&ainext, &hints, &in6addr, ASIO_OS_DEF(AF_INET6));
        if (rc != 0) {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
          return rc;
    ...
    char hbuf[8192] = "";
    hostent *hptr =
        socket_ops::gethostbyname(sptr->host, sptr->family, &hent, hbuf,
                                  sizeof(hbuf), hints.ai_flags, ec);
    if (hptr == 0) {
      if (search_count == 2) {
...
inline asio::error_code getnameinfo_emulation(const socket_addr_type *sa,
                                              std::size_t salen, char *host,
                                              std::size_t hostlen, char *serv,
                                              std::size_t servlen, int flags,
                                              asio::error_code &ec) {
  using namespace std;
  ...
  if (host && hostlen > 0) {
    if (flags & NI_NUMERICHOST) {
      if (socket_ops::inet_ntop(sa->sa_family, addr, host, hostlen, 0, ec) ==
          0) {
        return ec;
      }
    } else {
      hostent hent;
      char hbuf[8192] = "";
      hostent *hptr = socket_ops::gethostbyaddr(
          addr, static_cast<int>(addr_len), sa->sa_family, &hent, hbuf,
          sizeof(hbuf), ec);
...
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
    return asio::error_code(WSAGetLastError(),
...
asio::error_code getaddrinfo(const char *host, const char *service,
  ...
  typedef int(WSAAPI * gai_t)(const char *, const char *, const addrinfo_type *,
                              addrinfo_type **);
  if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
  ...
  if (error == 0 && service &&
      isdigit(static_cast<unsigned char>(service[0]))) {
...
asio::error_code getnameinfo(const void *addr, std::size_t addrlen, char *host,
                             std::size_t hostlen, char *serv,
                             std::size_t servlen, int flags,
                             asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  ...
  int error = ::getnameinfo(static_cast<const socket_addr_type *>(addr),
                            static_cast<socklen_t>(addrlen), host,
                            static_cast<DWORD>(hostlen), serv,
                            static_cast<DWORD>(servlen), flags);
  return ec = translate_addrinfo_error(error);
#else
  // Building for Windows 2000 or earlier.
  typedef int(WSAAPI * gni_t)(const socket_addr_type *, int, char *, DWORD,
                              char *, DWORD, int);
  if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
    if (gni_t gni = (gni_t)::GetProcAddress(winsock_module, "getnameinfo")) {
      clear_last_error();
      int error =
          gni(static_cast<const socket_addr_type *>(addr),
...
asio::error_code sync_getnameinfo(const void *addr, std::size_t addrlen,
  ...
  socket_ops::getnameinfo(addr, addrlen, host, hostlen, serv, servlen, flags,
                          ec);
  if (ec) {
    socket_ops::getnameinfo(addr, addrlen, host, hostlen, serv, servlen,
                            flags | NI_NUMERICSERV, ec);
  }
...
background_getnameinfo(const weak_cancel_token_type &cancel_token,
                       const void *addr, std::size_t addrlen, char *host,
                       std::size_t hostlen, char *serv, std::size_t servlen,
                       int sock_type, asio::error_code &ec) {
  if (cancel_token.expired()) {
    ec = asio::error::operation_aborted;
  } else {
    // First try resolving with the service name. If that fails try resolving
