...
#if defined(ASIO_HAS_FILE) && defined(ASIO_HAS_WINDOWS_RANDOM_ACCESS_HANDLE)
...
win_iocp_file_service::win_iocp_file_service(execution_context &context)
    : execution_context_service_base<win_iocp_file_service>(context),
      handle_service_(context), nt_flush_buffers_file_ex_(0) {
...
void win_iocp_file_service::shutdown() { handle_service_.shutdown(); }

asio::error_code
win_iocp_file_service::open(win_iocp_file_service::implementation_type &impl,
...
asio::error_code
win_iocp_file_service::resize(win_iocp_file_service::implementation_type &impl,
...
asio::error_code win_iocp_file_service::sync_all(
    win_iocp_file_service::implementation_type &impl, asio::error_code &ec) {
...
asio::error_code win_iocp_file_service::sync_data(
  ...
  if (nt_flush_buffers_file_ex_) {
    ...
    if (!nt_flush_buffers_file_ex_(native_handle(impl),
                                   flush_flags_file_data_sync_only, 0, 0,
                                   &status)) {
      asio::error::clear(ec);
      return ec;
    }
...
uint64_t
win_iocp_file_service::seek(win_iocp_file_service::implementation_type &impl,
                            int64_t offset, file_base::seek_basis whence,
                            asio::error_code &ec) {
  DWORD method;
  switch (whence) {
