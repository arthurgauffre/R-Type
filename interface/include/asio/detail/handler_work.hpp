...
template <typename Executor, typename CandidateExecutor, typename IoContext,
...
class handler_work_base<
    Executor, void, IoContext, PolymorphicExecutor,
...
public:
  ...
  handler_work_base(bool base1_owns_work, const executor_type &ex,
      ...
      : executor_(!base1_owns_work && ex == candidate
                      ? executor_type()
                      : asio::prefer(ex, execution::outstanding_work.tracked)) {
  }
...
class handler_work_base<
    Executor, CandidateExecutor, IoContext, PolymorphicExecutor,
    enable_if_t<is_same<Executor, any_completion_executor>::value ||
