...
public:
  io_uring_socket_recv_op_base(const asio::error_code &success_ec,
      ...
      : io_uring_operation(
            success_ec, &io_uring_socket_recv_op_base::do_prepare,
  ...
  static void do_prepare(io_uring_operation *base, ::io_uring_sqe *sqe) {
    ...
    if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
      ...
      ::io_uring_prep_poll_add(sqe, o->socket_, except_op ? POLLPRI : POLLIN);
               ...
               o->flags_ == 0) {
      ::io_uring_prep_read_fixed(sqe, o->socket_, o->bufs_.buffers()->iov_base,
    ...
    if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
      ...
      if (after_completion || !except_op) {
        if (o->bufs_.is_single_buffer) {
          ...
          return socket_ops::non_blocking_recv(
...
public:
  ...
  io_uring_socket_recv_op(const asio::error_code &success_ec, int socket,
                          socket_ops::state_type state,
                          const MutableBufferSequence &buffers,
                          socket_base::message_flags flags, Handler &handler,
                          const IoExecutor &io_ex)
      : io_uring_socket_recv_op_base<MutableBufferSequence>(
