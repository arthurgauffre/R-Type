...
protected:
  ...
  base_from_cancellation_state(const Handler &handler, InFilter &&in_filter,
  ...
  void reset_cancellation_state(const Handler &handler) {
    cancellation_state_ =
        cancellation_state(asio::get_associated_cancellation_slot(handler));
  }
  ...
  template <typename InFilter, typename OutFilter>
  void reset_cancellation_state(const Handler &handler, InFilter &&in_filter,
                                OutFilter &&out_filter) {
    cancellation_state_ =
        cancellation_state(asio::get_associated_cancellation_slot(handler),
                           static_cast<InFilter &&>(in_filter),
