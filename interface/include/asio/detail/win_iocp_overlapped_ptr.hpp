...
public:
  ...
  void reset(const Executor &ex, Handler handler) {
    ...
    ASIO_HANDLER_CREATION(
        (ex.context(), *p.p, "iocp_service", iocp_service, 0, "overlapped"));
...
private:
  ...
  static win_iocp_io_context *get_iocp_service(
      const Executor &ex,
      enable_if_t<can_query<const Executor &, execution::context_t>::value> * =
          0) {
  ...
  static win_iocp_io_context *
  get_iocp_service(const io_context::executor_type &ex) {
