...
public:
  ...
  void async_wait(base_implementation_type &impl, socket_base::wait_type w,
    ...
    ASIO_HANDLER_CREATION((reactor_.context(), *p.p, "socket", &impl,
    ...
    default:
      ...
      start_op(impl, reactor::read_op, p.p, is_continuation, false, true,
               &io_ex, 0);
      p.v = p.p = 0;
      return;
    }
  ...
  size_t send(base_implementation_type &impl,
    ...
    if (bufs_type::is_single_buffer) {
      return socket_ops::sync_send1(
          impl.socket_, impl.state_, bufs_type::first(buffers).data(),
  ...
  void async_send(base_implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef reactive_socket_send_op<ConstBufferSequence, Handler, IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
  ...
  void async_receive(base_implementation_type &impl, const null_buffers &,
    ...
    ASIO_HANDLER_CREATION((reactor_.context(), *p.p, "socket", &impl,
                           impl.socket_, "async_receive(null_buffers)"));
  ...
  void async_receive_with_flags(base_implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef reactive_socket_recvmsg_op<MutableBufferSequence, Handler,
                                       IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    p.p = new (p.v) op(success_ec_, impl.socket_, buffers, in_flags, out_flags,
                       handler, io_ex);
    ...
    start_op(impl,
             (in_flags & socket_base::message_out_of_band) ? reactor::except_op
                                                           : reactor::read_op,
             p.p, is_continuation,
             (in_flags & socket_base::message_out_of_band) == 0, false, &io_ex,
             0);
    p.v = p.p = 0;
  }
...
protected:
  ...
  ASIO_DECL asio::error_code do_open(base_implementation_type &impl, int af,
  ...
  ASIO_DECL void
  do_start_op(base_implementation_type &impl, int op_type, reactor_op *op,
  ...
  template <typename Op>
  void
  start_op(base_implementation_type &impl, int op_type, Op *op,
           bool is_continuation, bool is_non_blocking, bool noop, const void *,
           enable_if_t<is_same<
               typename associated_immediate_executor<
                   typename Op::handler_type, typename Op::io_executor_type>::
                   asio_associated_immediate_executor_is_unspecialised,
               void>::value> *) {
    return do_start_op(impl, op_type, op, is_continuation, is_non_blocking,
                       noop, &reactor::call_post_immediate_completion,
                       &reactor_);
  }

  // Start the asynchronous accept operation.
  ASIO_DECL void
  do_start_accept_op(base_implementation_type &impl, reactor_op *op,
  ...
  void start_accept_op(
      base_implementation_type &impl, Op *op, bool is_continuation,
      bool peer_is_open, const void *,
      enable_if_t<is_same<
          typename associated_immediate_executor<
              typename Op::handler_type, typename Op::io_executor_type>::
              asio_associated_immediate_executor_is_unspecialised,
          void>::value> *) {
    return do_start_accept_op(impl, op, is_continuation, peer_is_open,
                              &reactor::call_post_immediate_completion,
                              &reactor_);
  }

  // Start the asynchronous connect operation.
  ASIO_DECL void
  do_start_connect_op(base_implementation_type &impl, reactor_op *op,
                      bool is_continuation, const void *addr, size_t addrlen,
                      void (*on_immediate)(operation *op, bool, const void *),
                      const void *immediate_arg);
  ...
  void start_connect_op(base_implementation_type &impl, Op *op,
                        bool is_continuation, const void *addr, size_t addrlen,
                        const void *io_ex, ...) {
    return do_start_connect_op(impl, op, is_continuation, addr, addrlen,
                               &Op::do_immediate, io_ex);
  }
  ...
  void start_connect_op(
      base_implementation_type &impl, Op *op, bool is_continuation,
      const void *addr, size_t addrlen, const void *,
      enable_if_t<is_same<
          typename associated_immediate_executor<
              typename Op::handler_type, typename Op::io_executor_type>::
              asio_associated_immediate_executor_is_unspecialised,
  ...
  public:
    reactor_op_cancellation(reactor *r, reactor::per_descriptor_data *p,
                            socket_type d, int o)
    ...
    void operator()(cancellation_type_t type) {
      if (!!(type & (cancellation_type::terminal | cancellation_type::partial |
        ...
        reactor_->cancel_ops_by_key(descriptor_, *reactor_data_, op_type_,
                                    this);
      }
    }
