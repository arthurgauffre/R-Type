...
public:
  ...
  struct implementation_type
    ...
    // Default constructor.
    implementation_type()
        : protocol_(endpoint_type().protocol()), have_remote_endpoint_(false),
          remote_endpoint_() {}
  ...
  void async_send_to(implementation_type &impl,
    ...
    ASIO_HANDLER_CREATION(
        (context_, *p.p, "socket", &impl, impl.socket_, "async_send_to"));
    ...
    start_send_to_op(impl, bufs.buffers(), bufs.count(), destination.data(),
                     static_cast<int>(destination.size()), flags, o);
    p.v = p.p = 0;
  }
  ...
  void async_receive_from(implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef win_iocp_socket_recvfrom_op<MutableBufferSequence, endpoint_type,
                                        Handler, IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    operation *o = p.p =
        new (p.v) op(sender_endp, impl.cancel_token_, buffers, handler, io_ex);
  ...
  void async_accept(implementation_type &impl, Socket &peer,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef win_iocp_socket_accept_op<Socket, protocol_type, Handler,
                                      IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    ...
    operation *o = p.p =
        new (p.v) op(*this, impl.socket_, peer, impl.protocol_, peer_endpoint,
                     enable_connection_aborted, handler, io_ex);

    ASIO_HANDLER_CREATION(
        (context_, *p.p, "socket", &impl, impl.socket_, "async_accept"));
  ...
  void async_move_accept(implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef win_iocp_socket_move_accept_op<protocol_type, PeerIoExecutor,
                                           Handler, IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    bool enable_connection_aborted =
        (impl.state_ & socket_ops::enable_connection_aborted) != 0;
    operation *o = p.p =
        new (p.v) op(*this, impl.socket_, impl.protocol_, peer_io_ex,
    ...
    start_accept_op(impl, false, p.p->new_socket(), impl.protocol_.family(),
                    ...
                    p.p->output_buffer(), p.p->address_length(), o);
    p.v = p.p = 0;
  }
  ...
  void async_connect(implementation_type &impl,
    ...
    ASIO_HANDLER_CREATION(
        (context_, *p.p, "socket", &impl, impl.socket_, "async_connect"));
