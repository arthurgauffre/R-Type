...
public:
  ...
  enable_if_t<!is_same<decay_t<Arg>, asio::error_code>::value>
...
inline bool
asio_handler_is_continuation(redirect_error_handler<Handler> *this_handler) {
...
struct async_result<redirect_error_t<CompletionToken>, Signature>
  ...
  struct init_wrapper {
    explicit init_wrapper(asio::error_code &ec) : ec_(ec) {}

    template <typename Handler, typename Initiation, typename... Args>
    void operator()(Handler &&handler, Initiation &&initiation,
                    Args &&...args) const {
