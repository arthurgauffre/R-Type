...
inline typename Protocol::endpoint deref_connect_result(Iterator iter,
                                                        asio::error_code &ec) {
...
template <typename T, typename Iterator> struct is_legacy_connect_condition {
  ...
  static const bool value =
      sizeof(asio_connect_condition_check((
          ...
          declval<const asio::error_code>(), declval<const Iterator>()))) != 1;
};

template <typename ConnectCondition, typename Iterator>
inline Iterator call_connect_condition(
    ConnectCondition &connect_condition, const asio::error_code &ec,
    Iterator next, Iterator end,
    enable_if_t<is_legacy_connect_condition<ConnectCondition, Iterator>::value>
        * = 0) {
  if (next != end)
...
connect(basic_socket<Protocol, Executor> &s, const EndpointSequence &endpoints,
...
protected:
  ...
  void check_condition(const asio::error_code &ec, Iterator &iter,
                       Iterator &end) {
...
public:
  range_connect_op(basic_socket<Protocol, Executor> &sock,
      ...
      : base_from_cancellation_state<RangeConnectHandler>(
        ...
        socket_(sock), endpoints_(endpoints), index_(0), start_(0),
  ...
  range_connect_op(const range_connect_op &other)
      : base_from_cancellation_state<RangeConnectHandler>(other),
        ...
        socket_(other.socket_), endpoints_(other.endpoints_),
        index_(other.index_), start_(other.start_), handler_(other.handler_) {}
...
public:
  ...
  template <typename RangeConnectHandler, typename EndpointSequence,
            typename ConnectCondition>
  void operator()(RangeConnectHandler &&handler,
    ...
    // RangeConnectHandler.
    ASIO_RANGE_CONNECT_HANDLER_CHECK(RangeConnectHandler, handler,
                                     typename Protocol::endpoint)
    type_check;

    non_const_lvalue<RangeConnectHandler> handler2(handler);
    range_connect_op<Protocol, Executor, EndpointSequence, ConnectCondition,
                     decay_t<RangeConnectHandler>>(
        socket_, endpoints, connect_condition,
        handler2.value)(asio::error_code(), 1);
  }
...
public:
  iterator_connect_op(basic_socket<Protocol, Executor> &sock,
      ...
      : base_from_cancellation_state<IteratorConnectHandler>(
            handler, enable_partial_cancellation()),
        base_from_connect_condition<ConnectCondition>(connect_condition),
        socket_(sock), iter_(begin), end_(end), start_(0),
  ...
  iterator_connect_op(const iterator_connect_op &other)
      : base_from_cancellation_state<IteratorConnectHandler>(other),
        base_from_connect_condition<ConnectCondition>(other),
        socket_(other.socket_), iter_(other.iter_), end_(other.end_),
        start_(other.start_), handler_(other.handler_) {}
...
public:
  ...
  template <typename IteratorConnectHandler, typename Iterator,
            typename ConnectCondition>
  void operator()(IteratorConnectHandler &&handler, Iterator begin,
                  Iterator end,
                  const ConnectCondition &connect_condition) const {
    // If you get an error on the following line it means that your
    // handler does not meet the documented type requirements for an
    // IteratorConnectHandler.
    ASIO_ITERATOR_CONNECT_HANDLER_CHECK(IteratorConnectHandler, handler,
                                        Iterator)
    type_check;

    non_const_lvalue<IteratorConnectHandler> handler2(handler);
    iterator_connect_op<Protocol, Executor, Iterator, ConnectCondition,
                        decay_t<IteratorConnectHandler>>(
        socket_, begin, end, connect_condition,
        handler2.value)(asio::error_code(), 1);
  }
...
template <template <typename, typename> class Associator, typename Protocol,
          typename Executor, typename EndpointSequence,
          typename ConnectCondition, typename RangeConnectHandler,
...
struct associator<
  ...
  get(const detail::range_connect_op<Protocol, Executor, EndpointSequence,
      ...
      -> decltype(Associator<RangeConnectHandler, DefaultCandidate>::get(
          h.handler_, c)) {
    return Associator<RangeConnectHandler, DefaultCandidate>::get(h.handler_,
                                                                  c);
  }
...
template <template <typename, typename> class Associator, typename Protocol,
          ...
          typename IteratorConnectHandler, typename DefaultCandidate>
struct associator<
    Associator,
    detail::iterator_connect_op<Protocol, Executor, Iterator, ConnectCondition,
                                IteratorConnectHandler>,
  ...
  get(const detail::iterator_connect_op<Protocol, Executor, Iterator,
                                        ConnectCondition,
                                        IteratorConnectHandler> &h) noexcept {
    return Associator<IteratorConnectHandler, DefaultCandidate>::get(
        h.handler_);
  }

  static auto get(const detail::iterator_connect_op<Protocol, Executor,
                                                    Iterator, ConnectCondition,
                                                    IteratorConnectHandler> &h,
                  const DefaultCandidate &c) noexcept
      -> decltype(Associator<IteratorConnectHandler, DefaultCandidate>::get(
          h.handler_, c)) {
...
async_connect(basic_socket<Protocol, Executor> &s,
  ...
  return async_initiate<RangeConnectToken,
      ...
      detail::initiate_async_range_connect<Protocol, Executor>(s), token,
...
inline auto async_connect(basic_socket<Protocol, Executor> &s, Iterator begin,
  ...
  return async_initiate<IteratorConnectToken, void(asio::error_code, Iterator)>(
      detail::initiate_async_iterator_connect<Protocol, Executor>(s), token,
...
template <typename Protocol, typename Executor, typename Iterator,
          ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, Iterator))
              IteratorConnectToken>
inline auto async_connect(basic_socket<Protocol, Executor> &s, Iterator begin,
                          Iterator end, IteratorConnectToken &&token)
    -> decltype(async_initiate<IteratorConnectToken,
                               void(asio::error_code, Iterator)>(
        declval<detail::initiate_async_iterator_connect<Protocol, Executor>>(),
...
async_connect(basic_socket<Protocol, Executor> &s,
              const EndpointSequence &endpoints,
              ConnectCondition connect_condition, RangeConnectToken &&token,
              constraint_t<is_endpoint_sequence<EndpointSequence>::value>)
    -> decltype(async_initiate<RangeConnectToken,
                               void(asio::error_code,
                                    typename Protocol::endpoint)>(
