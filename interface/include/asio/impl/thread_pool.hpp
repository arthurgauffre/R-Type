...
template <typename Allocator, unsigned int Bits>
inline bool
thread_pool::basic_executor_type<Allocator, Bits>::running_in_this_thread()
    const noexcept {
...
void thread_pool::basic_executor_type<Allocator, Bits>::do_execute(
    Function &&f, false_type) const {
  ...
  if ((bits_ & relationship_continuation) != 0) {
    ASIO_HANDLER_CREATION(
        (*pool_, *p.p, "thread_pool", pool_, 0, "execute(blk=never,rel=cont)"));
  } else {
    ASIO_HANDLER_CREATION(
        (*pool_, *p.p, "thread_pool", pool_, 0, "execute(blk=never,rel=fork)"));
  }

  pool_->scheduler_.post_immediate_completion(
      p.p, (bits_ & relationship_continuation) != 0);
  p.v = p.p = 0;
}
...
void thread_pool::basic_executor_type<Allocator, Bits>::do_execute(
  ...
  ASIO_HANDLER_CREATION(
      (*pool_, op, "thread_pool", pool_, 0, "execute(blk=always)"));
...
template <typename Allocator, unsigned int Bits>
inline thread_pool &
thread_pool::basic_executor_type<Allocator, Bits>::context() const noexcept {
  return *pool_;
}

template <typename Allocator, unsigned int Bits>
inline void thread_pool::basic_executor_type<Allocator, Bits>::on_work_started()
    const noexcept {
  pool_->scheduler_.work_started();
}
...
template <typename Allocator, unsigned int Bits>
inline void
thread_pool::basic_executor_type<Allocator, Bits>::on_work_finished()
    const noexcept {
