...
ASIO_SYNC_OP_VOID
serial_port_base::baud_rate::store(ASIO_OPTION_STORAGE &storage,
...
ASIO_SYNC_OP_VOID
serial_port_base::baud_rate::load(const ASIO_OPTION_STORAGE &storage,
                                  asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
ASIO_SYNC_OP_VOID
serial_port_base::flow_control::store(ASIO_OPTION_STORAGE &storage,
                                      asio::error_code &ec) const {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
ASIO_SYNC_OP_VOID
serial_port_base::flow_control::load(const ASIO_OPTION_STORAGE &storage,
                                     asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  if (storage.fOutX && storage.fInX) {
    value_ = software;
  } else if (storage.fOutxCtsFlow &&
             storage.fRtsControl == RTS_CONTROL_HANDSHAKE) {
    value_ = hardware;
  } else {
    value_ = none;
...
serial_port_base::parity::parity(serial_port_base::parity::type t) : value_(t) {
...
ASIO_SYNC_OP_VOID serial_port_base::parity::store(ASIO_OPTION_STORAGE &storage,
                                                  asio::error_code &ec) const {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  switch (value_) {
  case none:
...
ASIO_SYNC_OP_VOID
serial_port_base::parity::load(const ASIO_OPTION_STORAGE &storage,
                               asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
serial_port_base::stop_bits::stop_bits(serial_port_base::stop_bits::type t)
    : value_(t) {
...
ASIO_SYNC_OP_VOID
serial_port_base::stop_bits::store(ASIO_OPTION_STORAGE &storage,
                                   asio::error_code &ec) const {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  switch (value_) {
  case one:
...
ASIO_SYNC_OP_VOID
serial_port_base::stop_bits::load(const ASIO_OPTION_STORAGE &storage,
                                  asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
serial_port_base::character_size::character_size(unsigned int t) : value_(t) {
  if (t < 5 || t > 8) {
...
ASIO_SYNC_OP_VOID
serial_port_base::character_size::store(ASIO_OPTION_STORAGE &storage,
                                        asio::error_code &ec) const {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
ASIO_SYNC_OP_VOID
serial_port_base::character_size::load(const ASIO_OPTION_STORAGE &storage,
                                       asio::error_code &ec) {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  value_ = storage.ByteSize;
#else
  if ((storage.c_cflag & CSIZE) == CS5) {
    value_ = 5;
  } else if ((storage.c_cflag & CSIZE) == CS6) {
    value_ = 6;
  } else if ((storage.c_cflag & CSIZE) == CS7) {
    value_ = 7;
  } else if ((storage.c_cflag & CSIZE) == CS8) {
    value_ = 8;
  } else {
    // Hmmm, use 8 for now.
    value_ = 8;
