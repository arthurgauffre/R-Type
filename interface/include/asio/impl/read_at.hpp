...
class read_at_op : public base_from_cancellation_state<ReadHandler>,
...
template <typename AsyncRandomAccessReadDevice, typename MutableBufferSequence,
          typename MutableBufferIterator, typename CompletionCondition,
          typename ReadHandler>
inline void start_read_at_op(AsyncRandomAccessReadDevice &d, uint64_t offset,
                             const MutableBufferSequence &buffers,
                             const MutableBufferIterator &,
                             CompletionCondition &completion_condition,
                             ReadHandler &handler) {
...
inline auto async_read_at(AsyncRandomAccessReadDevice &d, uint64_t offset,
  ...
  return async_initiate<ReadToken, void(asio::error_code, std::size_t)>(
      detail::initiate_async_read_at<AsyncRandomAccessReadDevice>(d), token,
      offset, buffers,
      static_cast<CompletionCondition &&>(completion_condition));
}
...
public:
  read_at_streambuf_op(AsyncRandomAccessReadDevice &device, uint64_t offset,
      ...
      : base_from_cancellation_state<ReadHandler>(
            handler, enable_partial_cancellation()),
        base_from_completion_cond<CompletionCondition>(completion_condition),
        device_(device), offset_(offset), streambuf_(streambuf), start_(0),
        total_transferred_(0), handler_(static_cast<ReadHandler &&>(handler)) {}

  read_at_streambuf_op(const read_at_streambuf_op &other)
      : base_from_cancellation_state<ReadHandler>(other),
        base_from_completion_cond<CompletionCondition>(other),
        device_(other.device_), offset_(other.offset_),
        streambuf_(other.streambuf_), start_(other.start_),
      ...
      for (;;) {
        {
          ...
          device_.async_read_some_at(
              offset_ + total_transferred_, streambuf_.prepare(bytes_available),
...
public:
  ...
  void operator()(ReadHandler &&handler, uint64_t offset,
    ...
    read_at_streambuf_op<AsyncRandomAccessReadDevice, Allocator,
        ...
        device_, offset, *b, completion_cond2.value,
...
struct associator<
    Associator,
    detail::read_at_streambuf_op<AsyncRandomAccessReadDevice, Executor,
                                 CompletionCondition, ReadHandler>,
    DefaultCandidate> : Associator<ReadHandler, DefaultCandidate> {
  static typename Associator<ReadHandler, DefaultCandidate>::type
  get(const detail::read_at_streambuf_op<AsyncRandomAccessReadDevice, Executor,
                                         CompletionCondition, ReadHandler>
          &h) noexcept {
    return Associator<ReadHandler, DefaultCandidate>::get(h.handler_);
  }
