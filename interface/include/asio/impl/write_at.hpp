...
class write_at_op : public base_from_cancellation_state<WriteHandler>,
...
public:
  write_at_op(AsyncRandomAccessWriteDevice &device, uint64_t offset,
      ...
      : base_from_cancellation_state<WriteHandler>(
        ...
        device_(device), offset_(offset), buffers_(buffers), start_(0),
  ...
  write_at_op(const write_at_op &other)
      : base_from_cancellation_state<WriteHandler>(other),
        ...
        device_(other.device_), offset_(other.offset_),
        buffers_(other.buffers_), start_(other.start_),
...
template <typename AsyncRandomAccessWriteDevice, typename ConstBufferSequence,
          typename ConstBufferIterator, typename CompletionCondition,
          typename WriteHandler>
inline void start_write_at_op(AsyncRandomAccessWriteDevice &d, uint64_t offset,
                              const ConstBufferSequence &buffers,
                              const ConstBufferIterator &,
                              CompletionCondition &completion_condition,
  ...
  detail::write_at_op<AsyncRandomAccessWriteDevice, ConstBufferSequence,
                      ConstBufferIterator, CompletionCondition, WriteHandler>(
      d, offset, buffers, completion_condition, handler)(asio::error_code(), 0,
                                                         1);
}
...
struct associator<
  ...
  get(const detail::write_at_op<
      AsyncRandomAccessWriteDevice, ConstBufferSequence, ConstBufferIterator,
...
inline auto async_write_at(AsyncRandomAccessWriteDevice &d, uint64_t offset,
    ...
    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
        declval<
            detail::initiate_async_write_at<AsyncRandomAccessWriteDevice>>(),
        token, offset, buffers,
        static_cast<CompletionCondition &&>(completion_condition))) {
  return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
      detail::initiate_async_write_at<AsyncRandomAccessWriteDevice>(d), token,
      offset, buffers,
      static_cast<CompletionCondition &&>(completion_condition));
}
...
template <template <typename, typename> class Associator, typename Executor,
          typename WriteHandler, typename DefaultCandidate>
struct associator<Associator,
  ...
  static typename Associator<WriteHandler, DefaultCandidate>::type
  get(const detail::write_at_streambuf_op<Executor, WriteHandler> &h) noexcept {
    return Associator<WriteHandler, DefaultCandidate>::get(h.handler_);
  }
...
inline auto async_write_at(AsyncRandomAccessWriteDevice &d, uint64_t offset,
    ...
    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
        ...
        static_cast<CompletionCondition &&>(completion_condition))) {
  return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
      detail::initiate_async_write_at_streambuf<AsyncRandomAccessWriteDevice>(
          d),
      token, offset, &b,
      static_cast<CompletionCondition &&>(completion_condition));
}
