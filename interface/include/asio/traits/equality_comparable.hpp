...
struct equality_comparable_trait<
    T, void_t<decltype(static_cast<void>(static_cast<bool>(declval<const T>() ==
                                                           declval<const T>())),
  ...
  static constexpr bool is_noexcept =
      noexcept(declval<const T>() == declval<const T>()) &&noexcept(
          declval<const T>() != declval<const T>());
};
...
template <typename T, typename = void>
struct equality_comparable_trait
    : conditional_t<is_same<T, decay_t<T>>::value, no_equality_comparable,
