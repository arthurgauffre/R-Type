...
struct coro_cancellation_source {
  ...
  void reset_cancellation_state(InFilter &&in_filter, OutFilter &&out_filter) {
    state = cancellation_state(slot, static_cast<InFilter &&>(in_filter),
...
template <> struct coro_error<std::exception_ptr> {
  static std::exception_ptr invalid() {
    return std::make_exception_ptr(
        asio::system_error(coro_error<asio::error_code>::invalid()));
  }

  static std::exception_ptr cancelled() {
    return std::make_exception_ptr(
        asio::system_error(coro_error<asio::error_code>::cancelled()));
  }

  static std::exception_ptr interrupted() {
    return std::make_exception_ptr(
        asio::system_error(coro_error<asio::error_code>::interrupted()));
  }

  static std::exception_ptr done() {
    return std::make_exception_ptr(
        asio::system_error(coro_error<asio::error_code>::done()));
  }
};
  ...
  struct awaitable_t {
    ...
    auto await_suspend(coroutine_handle<coro_promise<Y, R, E, A>> h)
      ...
      if (hp.get_executor() == coro.get_executor()) {
        ...
        return dispatch_coroutine(coro.coro_->get_executor(),
...
struct coro_promise_exchange<YieldReturn, void, YieldReturn>
    : coro_awaited_from {
...
struct coro_promise final
  ...
  coro_promise(Executor executor, Args &&...args) noexcept
      : coro_promise_allocator<Allocator>(executor,
                                          std::forward<Args>(args)...),
        executor_(std::move(executor)) {}

  template <typename First, typename... Args>
  coro_promise(First &&f, Executor executor, Args &&...args) noexcept
      : coro_promise_allocator<Allocator>(f, executor,
                                          std::forward<Args>(args)...),
        executor_(std::move(executor)) {}
  ...
  template <typename InFilter, typename OutFilter>
  auto
  await_transform(this_coro::reset_cancellation_state_2_t<InFilter, OutFilter>
                      reset) noexcept {
    struct result {
      detail::coro_cancellation_source *src_;
      InFilter in_filter_;
    ...
    return result{cancel, static_cast<InFilter &&>(reset.in_filter),
  ...
  // exception.
  auto
  await_transform(this_coro::throw_if_cancelled_1_t throw_if_cancelled) noexcept
      requires(!is_noexcept) {
    struct result {
      detail::coro_cancellation_source *src_;
      bool value_;
  ...
  auto await_transform(Op &&op,
    ...
    using handler_type = typename detail::coro_completion_handler_type<
        signature>::template completion_handler<coro_promise>;
...
struct coro<Yield, Return, Executor, Allocator>::awaitable_t {
  ...
  template <typename Y, typename R, typename E, typename A>
  auto
  await_suspend(detail::coroutine_handle<detail::coro_promise<Y, R, E, A>> h)
    ...
    if (hp.get_executor() == coro_.get_executor()) {
      ...
      struct cancel_handler {
        std::shared_ptr<
            std::pair<cancellation_signal, detail::coro_cancellation_source>>
            st =
                std::make_shared<std::pair<cancellation_signal,
      ...
      auto hh = detail::coroutine_handle<detail::coro_promise<
          Yield, Return, Executor, Allocator>>::from_promise(*coro_.coro_);

      return detail::dispatch_coroutine(coro_.coro_->get_executor(),
                                        [hh]() mutable { hh.resume(); })
          .handle;
    }
  }
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume {
  ...
  {
    return [this, the_coro = coro_, h = std::forward<WaitHandler>(handler),
  ...
  template <typename E, typename WaitHandler>
  requires(!std::is_void_v<result_type>) auto handle(
      E exec, WaitHandler &&handler, std::true_type /* error is noexcept */,
      std::false_type /* result is void */) // noexcept
  {
    return [the_coro = coro_, h = std::forward<WaitHandler>(handler),
            exec = std::move(exec)]() mutable {
      assert(the_coro);
      ...
      the_coro->awaited_from =
          detail::post_coroutine(exec, std::move(h), the_coro->result_).handle;
      the_coro->reset_error();
      ch.resume();
    };
  }
  ...
  auto handle(E exec, WaitHandler &&handler,
    ...
    return [the_coro = coro_, h = std::forward<WaitHandler>(handler),
      ...
      if (!ch)
        ...
        return asio::post(
            exec,
            asio::append(std::move(h), detail::coro_error<error_type>::done()));
      else {
        the_coro->awaited_from =
            detail::post_coroutine(exec, std::move(h), the_coro->error_).handle;
        the_coro->reset_error();
        ch.resume();
      }
    };
  ...
  auto handle(E exec, WaitHandler &&handler,
    ...
    return [the_coro = coro_, h = std::forward<WaitHandler>(handler),
      ...
      if (!ch)
        ...
        the_coro->awaited_from =
            detail::post_coroutine(exec, std::move(h), the_coro->error_,
                                   the_coro->result_)
                .handle;
        the_coro->reset_error();
        ch.resume();
  ...
  template <typename WaitHandler> void operator()(WaitHandler &&handler) {
    const auto exec =
        asio::prefer(get_associated_executor(handler, get_executor()),
                     execution::outstanding_work.tracked);
  ...
  void operator()(WaitHandler &&handler, Input &&input) {
    ...
    asio::dispatch(get_executor(),
                   [h = handle(exec, std::forward<WaitHandler>(handler),
                               std::integral_constant<bool, is_noexcept>{},
                               std::is_void<result_type>{}),
                    in = std::forward<Input>(input),
                    the_coro = coro_]() mutable {
