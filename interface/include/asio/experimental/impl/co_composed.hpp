...
template <typename Executors, typename Handler, typename Return, typename R,
          typename... Args>
class co_composed_state_return_overload<Executors, Handler, Return,
                                        R(Args...)> {
public:
  ...
  void on_cancellation_complete_with(Args... args) {
    ...
    state.cancellation_on_suspend_fn([](void *p) {
      ...
      co_composed_handler_base<Executors, Handler, Return> composed_handler(
          promise);
      ...
      co_composed_handler_base<Executors, Handler, Return>(
          std::move(composed_handler));
...
class co_composed_state_return<Executors, Handler,
                               co_composed_returns<Signatures...>>
    : public co_composed_state_return_overload<
          Executors, Handler, co_composed_returns<Signatures...>,
          Signatures>... {
public:
  using co_composed_state_return_overload<
      Executors, Handler, co_composed_returns<Signatures...>,
...
template <typename Executors, typename Handler, typename Return, typename R,
          typename... Args, typename... Signatures>
struct co_composed_state_default_cancellation_on_suspend_impl<
...
template <typename Executors, typename Handler, typename Return, typename R,
          typename... Args, typename... Signatures>
struct co_composed_state_default_cancellation_on_suspend_impl<
    Executors, Handler, Return, R(asio::error_code, Args...), Signatures...> {
  using promise_type = co_composed_promise<Executors, Handler, Return>;
  ...
  static constexpr void (*fn())(void *) {
    if constexpr ((is_constructible<Args>::value && ...)) {
      return [](void *p) {
        ...
        std::move(handler)(asio::error_code(asio::error::operation_aborted),
                           Args{}...);
      };
    } else {
      return co_composed_state_default_cancellation_on_suspend_impl<
...
template <typename Executors, typename Handler, typename Return, typename R,
          typename... Args, typename... Signatures>
struct co_composed_state_default_cancellation_on_suspend_impl<
    Executors, Handler, Return, R(std::exception_ptr, Args...), Signatures...> {
  using promise_type = co_composed_promise<Executors, Handler, Return>;
  ...
  static constexpr void (*fn())(void *) {
    if constexpr ((is_constructible<Args>::value && ...)) {
      return [](void *p) {
        ...
        std::move(handler)(std::make_exception_ptr(asio::system_error(
...
public:
  ...
  void reset_cancellation_state() {
    cancellation_state_ = cancellation_state(
        (get_associated_cancellation_slot)(static_cast<co_composed_state<
...
private:
  ...
  bool check_for_cancellation_on_suspend(
    ...
    if (complete_if_cancelled_ && !!cancelled() &&
        cancellation_on_suspend_fn_) {
  ...
  void (*cancellation_on_suspend_fn_)(void *) =
      co_composed_state_default_cancellation_on_suspend<Executors, Handler,
                                                        Return>::fn();
...
template <typename Executors, typename Handler, typename Return>
requires is_same <
    typename associated_cancellation_slot<Handler, cancellation_slot>::
        asio_associated_cancellation_slot_is_unspecialised,
void >
    ::value class co_composed_state_cancellation<Executors, Handler, Return> {
public:
  void reset_cancellation_state() {}
...
public:
  ...
  template <typename H>
  co_composed_state(composed_io_executors<Executors> &&executors, H &&h,
                    co_composed_on_suspend &on_suspend)
      : work_(std::move(executors)), handler_(std::forward<H>(h)),
  ...
  [[nodiscard]] co_composed_completion<Args...>
  complete(Args &&...args) requires requires {
    declval<Handler>()(std::forward<Args>(args)...);
  }
...
requires is_same <
    typename associated_cancellation_slot<Handler, cancellation_slot>::
        asio_associated_cancellation_slot_is_unspecialised,
void >
    ::value class co_composed_handler_cancellation<Executors, Handler, Return> {
};
...
public:
  using co_composed_handler_base<Executors, Handler,
                                 Return>::co_composed_handler_base;
...
template <typename Executors, typename Handler, typename Return, typename R,
          typename... Args>
class co_composed_handler<Executors, Handler, Return,
...
template <typename Executors, typename Handler, typename Return, typename R,
          typename... Args>
class co_composed_promise_return_overload<Executors, Handler, Return,
                                          R(Args...)> {
public:
...
template <typename Executors, typename Handler, typename... Signatures>
class co_composed_promise_return<Executors, Handler,
                                 co_composed_returns<Signatures...>>
    : public co_composed_promise_return_overload<
          Executors, Handler, co_composed_returns<Signatures...>,
          Signatures>... {
public:
...
public:
  ...
  void *operator new(std::size_t size,
                     co_composed_state<Executors, Handler, Return> &state,
                     Args &&...) {
    block_allocator_type allocator((
        get_associated_allocator)(state.handler_, recycling_allocator<void>()));
  ...
  void operator delete(void *ptr, std::size_t size) {
    block *base_ptr =
        static_cast<block *>(ptr) - blocks(sizeof(allocator_type));
    ...
    std::allocator_traits<block_allocator_type>::deallocate(
        block_allocator, base_ptr,
        blocks(sizeof(allocator_type)) + blocks(size));
  }

  template <typename... Args>
  co_composed_promise(co_composed_state<Executors, Handler, Return> &state,
                      Args &&...)
      : state_(state) {}
  ...
  auto await_transform(Op &&op
    ...
    public:
      awaitable(Op &&op, co_composed_promise &promise
          ...
          : op_(std::forward<Op>(op)), promise_(promise)
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
#if defined(ASIO_HAS_SOURCE_LOCATION)
            ,
            location_(location)
...
private:
  ...
  using block_allocator_type = typename std::allocator_traits<
      allocator_type>::template rebind_alloc<block>;
...
make_initiate_co_composed(Implementation &&implementation,
  ...
  return initiate_co_composed<decay_t<Implementation>, Executors,
                              Signatures...>(
...
template <completion_signature... Signatures, typename Implementation,
...
template <template <typename, typename> class Associator, typename Executors,
          typename Handler, typename Return, typename Signature,
          typename DefaultCandidate>
struct associator<Associator,
                  experimental::detail::co_composed_handler<Executors, Handler,
                                                            Return, Signature>,
  ...
  static auto
  get(const experimental::detail::co_composed_handler<Executors, Handler,
                                                      Return, Signature> &h,
      const DefaultCandidate &c) noexcept
      -> decltype(Associator<Handler, DefaultCandidate>::get(
...
template <typename C, typename Executors, typename Handler, typename Return,
          typename... Args>
struct coroutine_traits<
    void, C &,
    asio::experimental::detail::co_composed_state<Executors, Handler, Return>,
    Args...> {
  using promise_type =
      asio::experimental::detail::co_composed_promise<Executors, Handler,
                                                      Return>;
};
