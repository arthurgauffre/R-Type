...
struct promise_impl<void(Ts...), Executor, Allocator> {
  ...
  struct completion_impl final : completion_base {
    ...
    void invoke(Ts &&...ts) {
      ...
      using alloc_t =
          typename std::allocator_traits<typename asio::decay<Alloc>::type>::
              template rebind_alloc<completion_impl>;
  ...
  void set_completion(Alloc &&alloc, Handler &&handler) {
    ...
    completion = new (p)
        impl_t(std::forward<Alloc>(alloc), std::forward<Handler>(handler));
  }
...
struct promise_handler<void(Ts...), Executor, Allocator> {
  ...
  promise_handler(Allocator allocator,
                  Executor executor) // get_associated_allocator(exec)
      : impl_(std::allocate_shared<
              promise_impl<void(Ts...), Executor, Allocator>>(
  ...
  void operator()(std::remove_reference_t<Ts>... ts) {
    ...
    using result_type = typename promise_impl<void(Ts...), allocator_type,
                                              executor_type>::result_type;
