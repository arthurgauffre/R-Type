...
template <std::size_t N, typename R0, typename... Args0, typename R1,
          typename... Args1>
...
struct parallel_group_signature<N, Sig0, Sig1, SigN...> {
  typedef asio::detail::array<std::size_t, N> order_type;
  typedef typename parallel_group_signature<
      N, typename parallel_group_signature<N, Sig0, Sig1>::raw_type,
...
struct ranged_parallel_group_signature<R(Args...), Allocator> {
  typedef std::vector<std::size_t, ASIO_REBIND_ALLOC(Allocator, std::size_t)>
      order_type;
...
#else
  enum { value = detail::parallel_group_has_iterator_typedef<T>::value };
#endif
};
...
private:
  struct initiate_async_wait {
    ...
    void operator()(Handler &&h, Condition &&c,
                    std::tuple<Ops...> &&ops) const {
      detail::parallel_group_launch(std::forward<Condition>(c),
                                    std::forward<Handler>(h), ops,
...
public:
  ...
  typedef typename detail::parallel_group_signature<
      sizeof...(Ops), completion_signature_of_t<Ops>...>::type signature;
  ...
  auto async_wait(CancellationCondition cancellation_condition,
      ...
      -> decltype(asio::async_initiate<CompletionToken, signature>(
          declval<initiate_async_wait>(), token,
          std::move(cancellation_condition), std::move(ops_))) {
    return asio::async_initiate<CompletionToken, signature>(
        initiate_async_wait(), token, std::move(cancellation_condition),
        std::move(ops_));
  }
};
...
private:
  struct initiate_async_wait {
    template <typename Handler, typename Condition>
    void operator()(Handler &&h, Condition &&c, Range &&range,
                    const Allocator &allocator) const {
      detail::ranged_parallel_group_launch(
          std::move(c), std::move(h), std::forward<Range>(range), allocator);
    }
  };
...
public:
  ...
  auto async_wait(CancellationCondition cancellation_condition,
      ...
      -> decltype(asio::async_initiate<CompletionToken, signature>(
          declval<initiate_async_wait>(), token,
          std::move(cancellation_condition), std::move(range_), allocator_)) {
    return asio::async_initiate<CompletionToken, signature>(
        initiate_async_wait(), token, std::move(cancellation_condition),
