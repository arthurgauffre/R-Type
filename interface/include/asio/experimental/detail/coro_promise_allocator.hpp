...
void *allocate_coroutine(const std::size_t size, AllocatorType alloc_) {
  using alloc_type = typename std::allocator_traits<
      AllocatorType>::template rebind_alloc<unsigned char>;
  alloc_type alloc{alloc_};

  const auto align_needed = size % alignof(alloc_type);
  const auto align_offset =
      align_needed != 0 ? alignof(alloc_type) - align_needed : 0ull;
...
requires(Idx <= sizeof...(Args)) constexpr decltype(auto)
    get_variadic(First &&first, Args &&...args) {
  if constexpr (Idx == 0u)
...
template <typename Allocator> struct coro_promise_allocator {
  ...
  void *operator new(const std::size_t size, Args &...args) {
    return allocate_coroutine(
        size,
        get_variadic<
            variadic_first<std::allocator_arg_t, std::decay_t<Args>...>() + 1u>(
            args...));
  }
  ...
  coro_promise_allocator(Args &&...args)
      : alloc_(get_variadic<
               variadic_first<std::allocator_arg_t, std::decay_t<Args>...>() +
               1u>(args...)) {}
