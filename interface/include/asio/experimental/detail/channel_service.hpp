...
class channel_service : public asio::detail::execution_context_service_base<
                            channel_service<Mutex>> {
public:
  // Possible states for a channel end.
  enum state { buffer = 0, waiter = 1, block = 2, closed = 3 };

  // The base implementation type of all channels.
  struct base_implementation_type {
    // Default constructor.
    base_implementation_type()
        : receive_state_(block), send_state_(block), max_buffer_size_(0),
          next_(0), prev_(0) {}
  ...
  template <typename Message, typename Traits, typename... Signatures,
            typename... Args>
  ...
  template <typename Traits, typename... Signatures, typename Handler,
            typename IoExecutor>
  void
  async_send(implementation_type<Traits, Signatures...> &impl,
    ...
    if (slot.is_connected()) {
      p.p->cancellation_key_ =
          &slot.template emplace<op_cancellation<Traits, Signatures...>>(this,
                                                                         &impl);
    }

    ASIO_HANDLER_CREATION(
        (this->context(), *p.p, "channel", &impl, 0, "async_send"));
  ...
  void async_receive(implementation_type<Traits, Signatures...> &impl,
    ...
    ASIO_HANDLER_CREATION(
        (this->context(), *p.p, "channel", &impl, 0, "async_receive"));
...
struct channel_service<Mutex>::implementation_type : base_implementation_type {
  ...
  typedef conditional_t<
      has_signature<typename traits_type::receive_cancelled_signature,
                    Signatures...>::value,
      conditional_t<
          has_signature<typename traits_type::receive_closed_signature,
                        Signatures...>::value,
      ...
      conditional_t<
          has_signature<
              typename traits_type::receive_closed_signature, Signatures...,
...
struct channel_service<Mutex>::implementation_type<Traits, R()>
  ...
  typedef conditional_t<
      ...
      conditional_t<
          has_signature<
              typename traits_type::receive_closed_signature, R(),
              typename traits_type::receive_cancelled_signature>::value,
          channel_payload<R(),
                          typename traits_type::receive_cancelled_signature>,
          channel_payload<R(),
...
struct channel_service<Mutex>::implementation_type<Traits, R(asio::error_code)>
  ...
  // The traits type associated with the channel.
  typedef
      typename Traits::template rebind<R(asio::error_code)>::other traits_type;

  // Type of an element stored in the buffer.
  typedef conditional_t<
      has_signature<typename traits_type::receive_cancelled_signature,
                    R(asio::error_code)>::value,
      conditional_t<
          has_signature<typename traits_type::receive_closed_signature,
                        R(asio::error_code)>::value,
  ...
  // Construct with empty buffer.
  implementation_type() : size_(0) { first_.count_ = 0; }
