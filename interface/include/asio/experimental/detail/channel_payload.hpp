...
public:
  ...
  channel_payload(void_message_type &&)
      : message_(0, asio::error_code()), empty_(true) {}

  channel_payload(error_message_type &&m)
      : message_(static_cast<error_message_type &&>(m)), empty_(false) {}
...
public:
  ...
  channel_payload(message_1_type &&m) : index_(1) {
    new (&storage_.message_1_)
        message_1_type(static_cast<message_1_type &&>(m));
  }

  channel_payload(message_2_type &&m) : index_(2) {
    new (&storage_.message_2_)
        message_2_type(static_cast<message_2_type &&>(m));
  }

  channel_payload(channel_payload &&other) : index_(other.index_) {
    switch (index_) {
    case 1:
