...
template <typename T> struct promise_value_type<T> { using type = T; };

template <> struct promise_value_type<> { using type = std::tuple<>; };
...
private:
  ...
  promise(
      std::shared_ptr<detail::promise_impl<void(Ts...), Executor, Allocator>>
          impl)
      : impl_(impl) {}
  ...
  struct initiate_async_wait {
    ...
    void operator()(WaitHandler &&handler) const {
      const auto alloc =
          get_associated_allocator(handler, self_->get_allocator());

      auto cancel = get_associated_cancellation_slot(handler);

      if (self_->done) {
        auto exec =
            asio::get_associated_executor(handler, self_->get_executor());
        ...
        if (cancel.is_connected()) {
          struct cancel_handler {
            std::weak_ptr<
                detail::promise_impl<void(Ts...), Executor, Allocator>>
                self;
