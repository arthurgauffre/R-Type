 ...
 */
component::BackgroundComponent::BackgroundComponent(uint32_t entityID, const sf::Texture &texture, const sf::Vector2f &size)
    : AComponent(entityID), _size(size), _sprite(texture)
{
    _sprite.setScale(size.x / _sprite.getTexture()->getSize().x, size.y / _sprite.getTexture()->getSize().y);
 ...
 */
sf::Sprite component::BackgroundComponent::getSprite()
{
    return _sprite;
}
 ...
 */
const sf::Vector2f component::BackgroundComponent::getSize()
{
    return _size;
}
