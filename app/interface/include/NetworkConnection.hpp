...
private:
  ...
  void AddMessageToIncomingQueue() {
    if (ownerType == actualOwner::SERVER)
      queueOfIncomingMessages
          .pushBack({this->shared_from_this(),
                     temporaryIncomingMessage}) else queueOfIncomingMessages
          .pushBack({nullptr, temporaryIncomingMessage}) ReadHeader();
  }
  ...
  void ReadBody(){asioSocket.async_receive_from(
