...
namespace rtype
{
    namespace network
    {
        ...
        public:
            ...
            NetworkConnection(actualOwner owner, asio::io_context &otherAsioContext,
                              ...
                              rtype::network::ServerQueue<rtype::network::OwnedMessage<T>>
                                  &incomingMessages) : asioSocket(std::move(socket)),
        ...
        protected:
            ...
            rtype::network::ServerQueue<rtype::network::OwnedMessage<T>> &queueOfIncomingMessages;
        ...
        private:
            void WriteHeader()
            {
                ...
                asioSocket.async_send_to(
                    asio::buffer(&queueOfIncomingMessages.front().header, sizeof(rtype::network::MessageHeader<T>)), endpoint,
            ...
            void WriteBody()
            {
                ...
                asioSocket.async_send_to(
                    ...
                    queueOfIncomingMessages.front().body.size()), endpoint,
                    [this](std::error_code ec, std::size_t bytesReceived) {
                        if (!ec) {
                            queueOfIncomingMessages.popFront();
                            if (!queueOfIncomingMessages.empty()) {
            ...
            void ReadHeader()
            {
                asioSocket.async_receive_from(
                    asio::buffer(&temporaryIncomingMessage.header, sizeof(rtype::network::MessageHeader<T>)), endpoint,
                    [this](std::error_code ec, std::size_t bytesReceived) {
                        if (!ec) {
                            if (temporaryIncomingMessage.header.size > 0) {
                                temporaryIncomingMessage.body.resize(temporaryIncomingMessage.header.size);
                                ReadBody();
                            } else {
            ...
            {
                asioSocket.async_receive_from(
                    asio::buffer(temporaryIncomingMessage.body.data(), temporaryIncomingMessage.body.size()), endpoint,
                    [this](std::error_code ec, std::size_t bytesReceived) {
                        if (!ec) {
                            AddMessageToIncomingQueue();
                        } else {
            ...
            {
                asioSocket.async_send_to(asio::buffer(&handshakeOut, sizeof(uint64_t)), endpoint,
                    [this](std::error_code ec, std::size_t bytesReceived) {
                        if (!ec) {
