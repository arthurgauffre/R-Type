...
public:
  ...
  composed_op(I &&impl, W &&work, H &&handler)
      : base_from_cancellation_state<Handler>(handler,
        ...
        impl_(static_cast<I &&>(impl)), work_(static_cast<W &&>(work)),
        handler_(static_cast<H &&>(handler)), invocations_(0) {}
  ...
  typedef typename composed_work_guard<typename Work::head_type>::executor_type
      io_executor_type;
...
public:
  ...
  void operator()(Handler &&handler, Impl &&impl) const {
    composed_op<decay_t<Impl>, composed_work<Executors>, decay_t<Handler>,
                Signature>(static_cast<Impl &&>(impl),
...
inline initiate_composed_op<Signature, Executors>
make_initiate_composed_op(composed_io_executors<Executors> &&executors) {
  return initiate_composed_op<Signature, Executors>(
      0, static_cast<composed_io_executors<Executors> &&>(executors));
}
...
template <template <typename, typename> class Associator, typename Impl,
...
struct associator<Associator,
  ...
  get(const detail::composed_op<Impl, Work, Handler, Signature> &h) noexcept {
