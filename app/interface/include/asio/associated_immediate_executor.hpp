...
template <typename E>
struct default_immediate_executor<
    E, enable_if_t<!execution::is_executor<E>::value>,
    enable_if_t<is_executor<E>::value>> {
  class type : public E {
  public:
    template <typename Executor1>
    explicit type(const Executor1 &e,
                  constraint_t<conditional_t<!is_same<Executor1, type>::value,
...
struct associated_immediate_executor_impl<
    T, E, void_t<typename T::immediate_executor_type>> {
...
template <typename T, typename E>
struct associated_immediate_executor_impl<
    T, E, enable_if_t<!has_immediate_executor_type<T>::value>,
...
ASIO_NODISCARD inline auto get_associated_immediate_executor(
    ...
    constraint_t<is_executor<Executor>::value ||
                 execution::is_executor<Executor>::value> = 0) noexcept
...
ASIO_NODISCARD inline typename associated_immediate_executor<
...
get_associated_immediate_executor(
  ...
  return associated_immediate_executor<
      T, typename ExecutionContext::executor_type>::get(t, ctx.get_executor());
}
...
struct associated_immediate_executor_forwarding_base<
    T, E,
    enable_if_t<is_same<typename associated_immediate_executor<T, E>::
...
#endif // !defined(GENERATING_DOCUMENTATION)
{
  ...
  static auto get(reference_wrapper<T> t, const Executor &ex) noexcept
      -> decltype(associated_immediate_executor<T, Executor>::get(t.get(),
                                                                  ex)) {
