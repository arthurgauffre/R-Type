...
public:
  ...
  void operator()(CompletionHandler &&handler,
                  enable_if_t<execution::is_executor<associated_executor_t<
                      decay_t<CompletionHandler>>>::value> * = 0) const {
    ...
    asio::prefer(asio::require(ex, execution::blocking.never),
                 execution::relationship.fork, execution::allocator(alloc))
        .execute(asio::detail::bind_handler(
  ...
  void operator()(CompletionHandler &&handler,
    ...
    ex.post(
        asio::detail::bind_handler(static_cast<CompletionHandler &&>(handler)),
        alloc);
  }
};
...
public:
  ...
  void operator()(
      ...
      enable_if_t<!detail::is_work_dispatcher_required<
          decay_t<CompletionHandler>, Executor>::value> * = 0) const {
    associated_allocator_t<decay_t<CompletionHandler>> alloc(
        (get_associated_allocator)(handler));

    asio::prefer(asio::require(ex_, execution::blocking.never),
                 execution::relationship.fork, execution::allocator(alloc))
        .execute(asio::detail::bind_handler(
            static_cast<CompletionHandler &&>(handler)));
  }
