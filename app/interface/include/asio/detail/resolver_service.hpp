...
public:
  ...
  results_type resolve(implementation_type &, const query_type &qry,
    ...
    socket_ops::getaddrinfo(qry.host_name().c_str(), qry.service_name().c_str(),
                            qry.hints(), &address_info, ec);
    auto_addrinfo auto_address_info(address_info);

    ASIO_ERROR_LOCATION(ec);
    return ec ? results_type()
              : results_type::create(address_info, qry.host_name(),
                                     qry.service_name());
  }
  ...
  results_type resolve(implementation_type &, const endpoint_type &endpoint,
    ...
    socket_ops::sync_getnameinfo(endpoint.data(), endpoint.size(), host_name,
                                 NI_MAXHOST, service_name, NI_MAXSERV,
