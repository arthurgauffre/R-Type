...
public:
  ...
  void async_wait(base_implementation_type &impl, socket_base::wait_type w,
    ...
    ASIO_HANDLER_CREATION(
        (context_, *p.p, "socket", &impl, impl.socket_, "async_wait"));

    // Optionally register for per-operation cancellation.
    operation *iocp_op = p.p;
    if (slot.is_connected()) {
      p.p->cancellation_key_ = iocp_op =
          &slot.template emplace<reactor_op_cancellation>(impl.socket_,
                                                          iocp_op);
    }
  ...
  void async_send(base_implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef win_iocp_socket_send_op<ConstBufferSequence, Handler, IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    operation *o = p.p =
        new (p.v) op(impl.cancel_token_, buffers, handler, io_ex);

    ASIO_HANDLER_CREATION(
        (context_, *p.p, "socket", &impl, impl.socket_, "async_send"));
  ...
  void async_receive(base_implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef win_iocp_socket_recv_op<MutableBufferSequence, Handler, IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    operation *o = p.p =
        new (p.v) op(impl.state_, impl.cancel_token_, buffers, handler, io_ex);

    ASIO_HANDLER_CREATION(
        (context_, *p.p, "socket", &impl, impl.socket_, "async_receive"));
  ...
  size_t receive_with_flags(base_implementation_type &impl,
    ...
    return socket_ops::sync_recvmsg(impl.socket_, impl.state_, bufs.buffers(),
                                    bufs.count(), in_flags, out_flags, ec);
  }
  ...
  void async_receive_with_flags(base_implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef win_iocp_socket_recvmsg_op<MutableBufferSequence, Handler,
                                       IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
    operation *o = p.p =
        new (p.v) op(impl.cancel_token_, buffers, out_flags, handler, io_ex);
  ...
  ASIO_DECL void restart_accept_op(socket_type s, socket_holder &new_socket,
...
protected:
  ...
  ASIO_DECL asio::error_code do_open(base_implementation_type &impl, int family,
  ...
  ASIO_DECL void start_send_op(base_implementation_type &impl, WSABUF *buffers,
                               ...
                               socket_base::message_flags flags, bool noop,
                               operation *op);
  ...
  ASIO_DECL void start_receive_from_op(base_implementation_type &impl,
                                       WSABUF *buffers,
                                       std::size_t buffer_count, void *addr,
                                       socket_base::message_flags flags,
                                       int *addrlen, operation *op);
  ...
  ASIO_DECL void start_reactor_op(base_implementation_type &impl, int op_type,
                                  reactor_op *op);

  // Start the asynchronous connect operation using the reactor.
  ASIO_DECL int start_connect_op(base_implementation_type &impl, int family,
                                 int type, const void *remote_addr,
  ...
  typedef BOOL(PASCAL *connect_ex_fn)(SOCKET, const socket_addr_type *, int,
  ...
  typedef LONG(NTAPI *nt_set_info_fn)(HANDLE, ULONG_PTR *, void *, ULONG,
                                      ULONG);
  ...
  ASIO_DECL void *interlocked_compare_exchange_pointer(void **dest, void *exch,
                                                       void *cmp);
  ...
  public:
    iocp_op_cancellation(SOCKET s, operation *target)
        : operation(&iocp_op_cancellation::do_complete), socket_(s),
          target_(target) {}
  ...
  public:
    accept_op_cancellation(SOCKET s, operation *target)
        : operation(&iocp_op_cancellation::do_complete), socket_(s),
          target_(target), cancel_requested_(0) {}
  ...
  public:
    reactor_op_cancellation(SOCKET s, operation *base)
        : operation(&reactor_op_cancellation::do_complete), socket_(s),
          target_(base), reactor_(0), reactor_data_(0), op_type_(-1) {}

    void use_reactor(select_reactor *r, select_reactor::per_descriptor_data *p,
                     int o) {
      reactor_ = r;
      reactor_data_ = p;
      op_type_ = o;
