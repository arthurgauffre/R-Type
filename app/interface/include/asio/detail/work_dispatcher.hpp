...
public:
  ...
  work_dispatcher(CompletionHandler &&handler, const Executor &handler_ex)
      : handler_(static_cast<CompletionHandler &&>(handler)),
        executor_(
            asio::prefer(handler_ex, execution::outstanding_work.tracked)) {}

  work_dispatcher(const work_dispatcher &other)
      : handler_(other.handler_), executor_(other.executor_) {}
...
private:
  typedef decay_t<prefer_result_t<const Executor &,
...
public:
  ...
  void operator()() {
    ...
    work_.get_executor().dispatch(
        asio::detail::bind_handler(static_cast<Handler &&>(handler_)), alloc);
    work_.reset();
  }
