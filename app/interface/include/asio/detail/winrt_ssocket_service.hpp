...
public:
  ...
  struct implementation_type : base_implementation_type {
    // Default constructor.
    implementation_type()
        : base_implementation_type(), protocol_(endpoint_type().protocol()) {}
  ...
  winrt_ssocket_service(execution_context &context)
      : execution_context_service_base<winrt_ssocket_service<Protocol>>(
            context),
  ...
  asio::error_code set_option(implementation_type &impl, const Option &option,
    ...
    return do_set_option(
        impl, option.level(impl.protocol_), option.name(impl.protocol_),
  ...
  void async_connect(implementation_type &impl,
    ...
    ASIO_HANDLER_CREATION(
        (scheduler_.context(), *p.p, "socket", &impl, 0, "async_connect"));
