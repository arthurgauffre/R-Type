...
public:
  ...
  explicit executor_function(F f, const Alloc &a) {
    ...
    typename impl_type::ptr p = {detail::addressof(a),
                                 impl_type::ptr::allocate(a), 0};
...
private:
  ...
  template <typename Function, typename Alloc> struct impl : impl_base {
    ...
    impl(F &&f, const Alloc &a)
        : function_(static_cast<F &&>(f)), allocator_(a) {
  ...
  static void complete(impl_base *base, bool call) {
    ...
    typename impl<Function, Alloc>::ptr p = {detail::addressof(allocator), i,
                                             i};
...
public:
  ...
  explicit executor_function_view(F &f) noexcept
      : complete_(&executor_function_view::complete<F>), function_(&f) {}
