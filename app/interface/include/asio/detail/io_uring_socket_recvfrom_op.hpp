...
public:
  io_uring_socket_recvfrom_op_base(const asio::error_code &success_ec,
      ...
      : io_uring_operation(
            success_ec, &io_uring_socket_recvfrom_op_base::do_prepare,
            &io_uring_socket_recvfrom_op_base::do_perform, complete_func),
        socket_(socket), state_(state), buffers_(buffers),
        sender_endpoint_(endpoint), flags_(flags), bufs_(buffers), msghdr_() {
    msghdr_.msg_iov = bufs_.buffers();
    msghdr_.msg_iovlen = static_cast<int>(bufs_.count());
    msghdr_.msg_name =
        static_cast<sockaddr *>(static_cast<void *>(sender_endpoint_.data()));
...
public:
  ...
  io_uring_socket_recvfrom_op(const asio::error_code &success_ec, int socket,
                              socket_ops::state_type state,
                              const MutableBufferSequence &buffers,
                              Endpoint &endpoint,
                              socket_base::message_flags flags,
