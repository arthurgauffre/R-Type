...
public:
  reactive_socket_send_op_base(const asio::error_code &success_ec,
      ...
      : reactor_op(success_ec, &reactive_socket_send_op_base::do_perform,
                   complete_func),
  ...
  static status do_perform(reactor_op *base) {
    ...
    if (bufs_type::is_single_buffer) {
      result = socket_ops::non_blocking_send1(
      ...
      result = socket_ops::non_blocking_send(o->socket_, bufs.buffers(),
                                             bufs.count(), o->flags_, o->ec_,
                                             o->bytes_transferred_)
                   ? done
                   : not_done;
