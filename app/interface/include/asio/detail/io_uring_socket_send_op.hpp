...
public:
  io_uring_socket_send_op_base(const asio::error_code &success_ec,
      ...
      : io_uring_operation(
            success_ec, &io_uring_socket_send_op_base::do_prepare,
            &io_uring_socket_send_op_base::do_perform, complete_func),
        socket_(socket), state_(state), buffers_(buffers), flags_(flags),
        bufs_(buffers), msghdr_() {
  ...
  static void do_prepare(io_uring_operation *base, ::io_uring_sqe *sqe) {
    ...
    if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
      ::io_uring_prep_poll_add(sqe, o->socket_, POLLOUT);
               ...
               o->flags_ == 0) {
      ::io_uring_prep_write_fixed(sqe, o->socket_, o->bufs_.buffers()->iov_base,
                                  o->bufs_.buffers()->iov_len, 0,
  ...
  static bool do_perform(io_uring_operation *base, bool after_completion) {
    ...
    if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
      if (o->bufs_.is_single_buffer) {
        return socket_ops::non_blocking_send1(
            o->socket_, o->bufs_.first(o->buffers_).data(),
            o->bufs_.first(o->buffers_).size(), o->flags_, o->ec_,
            o->bytes_transferred_);
      } else {
        return socket_ops::non_blocking_send(o->socket_, o->bufs_.buffers(),
                                             o->bufs_.count(), o->flags_,
...
public:
  ...
  io_uring_socket_send_op(const asio::error_code &success_ec, int socket,
                          socket_ops::state_type state,
                          const ConstBufferSequence &buffers,
                          socket_base::message_flags flags, Handler &handler,
                          const IoExecutor &io_ex)
      : io_uring_socket_send_op_base<ConstBufferSequence>(
            success_ec, socket, state, buffers, flags,
