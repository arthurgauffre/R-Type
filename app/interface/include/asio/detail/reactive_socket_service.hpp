...
public:
  ...
  void async_send_to(implementation_type &impl,
    ...
    p.p = new (p.v) op(success_ec_, impl.socket_, buffers, destination, flags,
                       handler, io_ex);
    ...
    start_op(impl, reactor::write_op, p.p, is_continuation, true, false, &io_ex,
             0);
    p.v = p.p = 0;
  }
  ...
  void async_receive_from(implementation_type &impl,
    ...
    // Allocate and construct an operation to wrap the handler.
    typedef reactive_socket_recvfrom_op<MutableBufferSequence, endpoint_type,
                                        Handler, IoExecutor>
        op;
    typename op::ptr p = {asio::detail::addressof(handler),
                          op::ptr::allocate(handler), 0};
