...
protected:
  static mutable_registered_buffer
  make_buffer(const mutable_buffer &b, const void *scope, int index) noexcept {
...
public:
  ...
  buffer_registration(const Executor &ex,
      ...
      : buffer_sequence_(buffer_sequence),
        buffers_(ASIO_REBIND_ALLOC(allocator_type,
  ...
  buffer_registration(
      ...
      : buffer_sequence_(buffer_sequence),
        buffers_(ASIO_REBIND_ALLOC(allocator_type,
                                   mutable_registered_buffer)(alloc)) {
    init_buffers(ctx, asio::buffer_sequence_begin(buffer_sequence_),
                 asio::buffer_sequence_end(buffer_sequence_));
  }
...
private:
  ...
  std::vector<mutable_registered_buffer,
              ASIO_REBIND_ALLOC(allocator_type, mutable_registered_buffer)>
      buffers_;
#if defined(ASIO_HAS_IO_URING)
...
ASIO_NODISCARD inline buffer_registration<MutableBufferSequence>
...
ASIO_NODISCARD inline buffer_registration<MutableBufferSequence, Allocator>
register_buffers(const Executor &ex,
                 const MutableBufferSequence &buffer_sequence,
