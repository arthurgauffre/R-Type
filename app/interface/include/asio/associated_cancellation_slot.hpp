...
struct associated_cancellation_slot_impl<
    T, S, void_t<typename T::cancellation_slot_type>> {
...
template <typename T, typename S>
struct associated_cancellation_slot_impl<
    T, S, enable_if_t<!has_cancellation_slot_type<T>::value>,
...
struct associated_cancellation_slot_forwarding_base<
    T, S,
    enable_if_t<is_same<typename associated_cancellation_slot<T, S>::
...
#endif // !defined(GENERATING_DOCUMENTATION)
{
  ...
  static auto get(reference_wrapper<T> t, const CancellationSlot &s) noexcept
      -> decltype(associated_cancellation_slot<T, CancellationSlot>::get(
          t.get(), s)) {
