...
public:
  ...
  explicit strand(
      const Executor1 &e,
      constraint_t<conditional_t<!is_same<Executor1, strand>::value,
  ...
  strand(const strand &other) noexcept
      : executor_(other.executor_), impl_(other.impl_) {}
  ...
  query(const Property &p) const
    ...
    return this->query_helper(is_convertible<Property, execution::blocking_t>(),
                              p);
  }
  ...
  constraint_t<traits::execute_member<const Executor &, Function>::is_valid,
               void>
  execute(Function &&f) const {
    detail::strand_executor_service::execute(impl_, executor_,
...
private:
  ...
  static implementation_type create_implementation(
      const InnerExecutor &ex,
      constraint_t<!can_query<InnerExecutor, execution::context_t>::value> =
          0) {
    return use_service<detail::strand_executor_service>(ex.context())
        .create_implementation();
  }

  strand(const Executor &ex, const implementation_type &impl)
      : executor_(ex), impl_(impl) {}

  template <typename Property>
  query_result_t<const Executor &, Property>
  query_helper(false_type, const Property &property) const {
    return asio::query(executor_, property);
  }

  template <typename Property>
  execution::blocking_t query_helper(true_type,
                                     const Property &property) const {
    execution::blocking_t result = asio::query(executor_, property);
    return result == execution::blocking.always ? execution::blocking.possibly
                                                : result;
  }
