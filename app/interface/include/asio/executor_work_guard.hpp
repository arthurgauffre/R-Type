...
template <typename Executor>
class executor_work_guard<
    Executor, enable_if_t<!is_executor<Executor>::value>,
...
public:
  ...
  explicit executor_work_guard(const executor_type &e) noexcept
      : executor_(e), owns_(true) {
    new (&work_)
        work_type(asio::prefer(executor_, execution::outstanding_work.tracked));
  }
  ...
  executor_work_guard(const executor_work_guard &other) noexcept
    ...
    if (owns_) {
      new (&work_) work_type(
          asio::prefer(executor_, execution::outstanding_work.tracked));
    }
  }
  ...
  executor_work_guard(executor_work_guard &&other) noexcept
      : executor_(static_cast<Executor &&>(other.executor_)),
        owns_(other.owns_) {
    if (owns_) {
      new (&work_) work_type(static_cast<work_type &&>(
          *static_cast<work_type *>(static_cast<void *>(&other.work_))));
      other.owns_ = false;
    }
  }
...
private:
  ...
  typedef decay_t<prefer_result_t<const executor_type &,
...
ASIO_NODISCARD inline executor_work_guard<associated_executor_t<T, Executor>>
...
make_work_guard(
    const T &t, ExecutionContext &ctx, constraint_t<!is_executor<T>::value> = 0,
