...
public:
  ...
  template <typename... Args> void operator()(Args &&...args) {
    static_cast<Tail &&>(tail_)(static_cast<Args &&>(args)...)(
...
public:
  ...
  auto operator()(CompletionToken &&token)
    ...
    return async_initiate<CompletionToken, Signatures...>(
        initiate(), token, static_cast<Head &&>(head_),
        static_cast<Tail &&>(tail_));
  }

  template <ASIO_COMPLETION_TOKEN_FOR(Signatures...) CompletionToken>
  auto operator()(CompletionToken &&token)
      const & -> decltype(async_initiate<CompletionToken, Signatures...>(
          initiate(), token, this->head_, this->tail_)) {
    return async_initiate<CompletionToken, Signatures...>(initiate(), token,
                                                          head_, tail_);
  }
};
...
template <typename Head, typename Tail> struct deferred_sequence_types {
  ...
  template <typename... Signatures> struct op2 {
    typedef typename deferred_sequence_signatures<
        Tail, Signatures...>::template apply<op1>::type::type type;
  };

  typedef
      typename completion_signature_of<Head>::template apply<op2>::type::type
          base;
};
...
public:
  ...
  auto operator()(Args &&...args) && -> decltype(static_cast<Function &&>(
      this->function_)(static_cast<Args &&>(args)...)) {
    return static_cast<Function &&>(function_)(static_cast<Args &&>(args)...);
  }

  template <typename... Args>
  auto operator()(Args &&...args)
      const & -> decltype(Function(function_)(static_cast<Args &&>(args)...)) {
...
#if !defined(GENERATING_DOCUMENTATION)
template <typename Signature, typename Initiation, typename... InitArgs>
struct is_deferred<deferred_async_operation<Signature, Initiation, InitArgs...>>
    : true_type {};
#endif // !defined(GENERATING_DOCUMENTATION)
...
private:
  ...
  explicit deferred_conditional(bool b, T &&on_true, F &&on_false)
      : on_true_(static_cast<T &&>(on_true)),
        on_false_(static_cast<F &&>(on_false)), bool_(b) {}
...
public:
  ...
  constexpr explicit deferred_conditional(bool b)
      : on_true_(), on_false_(), bool_(b) {}

  /// Invoke the conditional branch bsaed on the stored value.
  template <typename... Args>
  auto operator()(Args &&...args) && -> decltype(static_cast<OnTrue &&>(
      on_true_)(static_cast<Args &&>(args)...)) {
    if (bool_) {
  ...
  template <typename... Args>
  auto operator()(Args &&...args)
      const & -> decltype(on_true_(static_cast<Args &&>(args)...)) {
    if (bool_) {
  ...
  template <typename T>
  deferred_conditional<T, OnFalse>
  then(T on_true, constraint_t<is_deferred<T>::value> * = 0,
       constraint_t<is_same<conditional_t<true, OnTrue, T>,
                            deferred_noop>::value> * = 0) && {
    return deferred_conditional<T, OnFalse>(bool_, static_cast<T &&>(on_true),
  ...
  deferred_conditional<OnTrue, T> otherwise(
      ...
      constraint_t<
          !is_same<conditional_t<true, OnTrue, T>, deferred_noop>::value> * = 0,
      constraint_t<is_same<conditional_t<true, OnFalse, T>,
                           deferred_noop>::value> * = 0) && {
    return deferred_conditional<OnTrue, T>(
        bool_, static_cast<OnTrue &&>(on_true_), static_cast<T &&>(on_false));
  }
};
...
public:
  ...
  struct executor_with_default : InnerExecutor {
    ...
    executor_with_default(
        ...
        constraint_t<conditional_t<
            !is_same<InnerExecutor1, executor_with_default>::value,
            is_convertible<InnerExecutor1, InnerExecutor>, false_type>::value> =
            0) noexcept
        : InnerExecutor(ex) {}
  };
  ...
  static typename decay_t<T>::template rebind_executor<
      executor_with_default<typename decay_t<T>::executor_type>>::other
  as_default_on(T &&object) {
    return typename decay_t<T>::template rebind_executor<executor_with_default<
        typename decay_t<T>::executor_type>>::other(static_cast<T &&>(object));
  }

  /// Creates a new deferred from a function.
  template <typename Function>
  constraint_t<!is_deferred<decay_t<Function>>::value,
  ...
  template <typename T>
  constraint_t<is_deferred<decay_t<T>>::value, decay_t<T>>
  operator()(T &&t) const {
  ...
  static constexpr deferred_values<decay_t<Args>...> values(Args &&...args) {
    return deferred_values<decay_t<Args>...>(deferred_init_tag{},
...
template <typename Head, typename Tail>
inline auto operator|(Head head, Tail &&tail) -> constraint_t<
