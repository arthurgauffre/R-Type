...
struct are_any_lvalue_completion_signatures<Sig0, SigN...>
    : integral_constant<
          bool, (is_lvalue_completion_signature<Sig0>::value ||
...
struct are_any_rvalue_completion_signatures<Sig0, SigN...>
    : integral_constant<
          bool, (is_rvalue_completion_signature<Sig0>::value ||
...
private:
  completion_handler_async_result(const completion_handler_async_result &) =
      delete;
  completion_handler_async_result &
  operator=(const completion_handler_async_result &) = delete;
};
...
class async_result
    : public conditional_t<
          detail::are_any_lvalue_completion_signatures<Signatures...>::value ||
              !detail::are_any_rvalue_completion_signatures<
                  Signatures...>::value,
          detail::completion_handler_async_result<CompletionToken,
                                                  Signatures...>,
          async_result<CompletionToken,
                       typename detail::simple_completion_signature<
                           Signatures>::type...>> {
public:
...
struct async_completion {
  ...
  typedef typename asio::async_result<decay_t<CompletionToken>,
...
struct async_result_has_initiate_memfn
    : integral_constant<
          bool, sizeof(async_result_initiate_memfn_helper<async_result<
                           decay_t<CompletionToken>, Signatures...>>(0)) != 1> {
};
...
#define ASIO_INITFN_RESULT_TYPE2(ct, sig0, sig1)                               \
  typename ::asio::async_result<typename ::asio::decay<ct>::type, sig0,        \
                                sig1>::return_type
#define ASIO_INITFN_RESULT_TYPE3(ct, sig0, sig1, sig2)                         \
  typename ::asio::async_result<typename ::asio::decay<ct>::type, sig0, sig1,  \
                                sig2>::return_type
...
#define ASIO_HANDLER_TYPE2(ct, sig0, sig1)                                     \
  typename ::asio::async_result<typename ::asio::decay<ct>::type, sig0,        \
                                sig1>::completion_handler_type
#define ASIO_HANDLER_TYPE3(ct, sig0, sig1, sig2)                               \
  typename ::asio::async_result<typename ::asio::decay<ct>::type, sig0, sig1,  \
                                sig2>::completion_handler_type
#endif
...
void_or_deduced async_initiate(Initiation &&initiation,
...
inline auto async_initiate(Initiation &&initiation,
    ...
    -> constraint_t<
        detail::async_result_has_initiate_memfn<CompletionToken,
                                                Signatures...>::value,
        decltype(async_result<decay_t<CompletionToken>, Signatures...>::
                     initiate(static_cast<Initiation &&>(initiation),
                              static_cast<CompletionToken &&>(token),
                              static_cast<Args &&>(args)...))> {
  return async_result<decay_t<CompletionToken>, Signatures...>::initiate(
      static_cast<Initiation &&>(initiation),
      static_cast<CompletionToken &&>(token), static_cast<Args &&>(args)...);
}
...
struct is_async_operation_call<
    Call, void_t<enable_if_t<
              is_same<result_of_t<Call>, async_operation_probe_result>::value>>>
    : true_type {};
...
public:
  ...
  static return_type initiate(Initiation &&, detail::async_operation_probe,
                              InitArgs &&...) {
    return return_type();
  }
};
...
template <typename T, typename... Args>
struct is_async_operation : detail::is_async_operation_call<T(
                                Args..., detail::async_operation_probe)> {};
...
public:
  ...
  template <typename Initiation, typename... InitArgs>
  static return_type initiate(Initiation &&, detail::completion_signature_probe,
                              InitArgs &&...) {
    return return_type();
  }
};
...
struct default_completion_token_impl<
    T, void_t<typename T::default_completion_token_type>> {
