...
public:
  ...
  spawned_coroutine_thread(caller_type &caller)
      : caller_(caller), on_suspend_fn_(0), on_suspend_arg_(0) {}

  template <typename F>
  static spawned_thread_base *
  spawn(F &&f, const boost::coroutines::attributes &attributes,
  ...
  template <typename F>
  static spawned_thread_base *
  spawn(F &&f, cancellation_slot parent_cancel_slot = cancellation_slot(),
        cancellation_state cancel_state = cancellation_state()) {
...
public:
  ...
  spawned_fiber_thread(fiber_type &&caller)
      : caller_(static_cast<fiber_type &&>(caller)), on_suspend_fn_(0),
        on_suspend_arg_(0) {}

  template <typename StackAllocator, typename F>
  static spawned_thread_base *
  spawn(allocator_arg_t, StackAllocator &&stack_allocator, F &&f,
        cancellation_slot parent_cancel_slot = cancellation_slot(),
        cancellation_state cancel_state = cancellation_state()) {
...
public:
  ...
  static return_type initiate(Initiation &&init,
    ...
    yield.spawned_thread_->suspend_with([&]() {
      static_cast<Initiation &&>(init)(handler_type(yield, result),
...
public:
  ...
  spawn_entry_point(const Executor &ex, F &&f, H &&h)
      : executor_(ex), function_(static_cast<F &&>(f)),
        handler_(static_cast<H &&>(h)), work_(handler_, executor_) {}
...
private:
  void call(const basic_yield_context<Executor> &yield, void_type<void>) {
...
#endif // !defined(ASIO_NO_EXCEPTIONS)
    {
      ...
      if (!yield.spawned_thread_->has_context_switched())
        (post)(yield);
      detail::binder1<Handler, exception_ptr> handler(handler_,
                                                      exception_ptr());
  ...
  void call(const basic_yield_context<Executor> &yield, void_type<T>) {
...
#endif // !defined(ASIO_NO_EXCEPTIONS)
    {
      ...
      if (!yield.spawned_thread_->has_context_switched())
        (post)(yield);
      detail::binder2<Handler, exception_ptr, T> handler(
          handler_, exception_ptr(), static_cast<T &&>(result));
      work_.complete(handler, handler.handler_);
    }
#if !defined(ASIO_NO_EXCEPTIONS)
#if defined(ASIO_HAS_BOOST_CONTEXT_FIBER)
...
public:
  ...
  void operator()(Handler &&handler, F &&f) const {
    ...
    typedef spawn_cancellation_handler<handler_type, Executor>
        cancel_handler_type;
    ...
    (dispatch)(executor_,
               spawned_thread_resumer(default_spawned_thread_type::spawn(
  ...
  void operator()(Handler &&handler, allocator_arg_t,
                  StackAllocator &&stack_allocator, F &&f) const {
    typedef decay_t<Handler> handler_type;
    typedef decay_t<F> function_type;
    typedef spawn_cancellation_handler<handler_type, Executor>
        cancel_handler_type;
    ...
    (dispatch)(
        executor_,
        spawned_thread_resumer(spawned_fiber_thread::spawn(
            allocator_arg_t(), static_cast<StackAllocator &&>(stack_allocator),
            spawn_entry_point<Executor, function_type, handler_type>(
                executor_, static_cast<F &&>(f),
                static_cast<Handler &&>(handler)),
...
template <
    ...
    ASIO_COMPLETION_TOKEN_FOR(
        typename detail::spawn_signature<result_of_t<
            F(basic_yield_context<typename ExecutionContext::executor_type>)>>::
            type) CompletionToken>
inline auto
spawn(ExecutionContext &ctx, F &&function, CompletionToken &&token,
#if defined(ASIO_HAS_BOOST_COROUTINE)
      constraint_t<!is_same<decay_t<CompletionToken>,
                            boost::coroutines::attributes>::value>,
#endif // defined(ASIO_HAS_BOOST_COROUTINE)
    ...
    -> decltype(async_initiate<
                CompletionToken,
                typename detail::spawn_signature<result_of_t<
                    F(basic_yield_context<
                        typename ExecutionContext::executor_type>)>>::type>(
        declval<
            detail::initiate_spawn<typename ExecutionContext::executor_type>>(),
...
template <typename Executor, typename F,
          ASIO_COMPLETION_TOKEN_FOR(
              typename detail::spawn_signature<result_of_t<
                  F(basic_yield_context<Executor>)>>::type) CompletionToken>
inline auto spawn(const basic_yield_context<Executor> &ctx, F &&function,
...
template <
    typename ExecutionContext, typename StackAllocator, typename F,
    ASIO_COMPLETION_TOKEN_FOR(
        typename detail::spawn_signature<result_of_t<
            F(basic_yield_context<typename ExecutionContext::executor_type>)>>::
            type) CompletionToken>
inline auto
spawn(ExecutionContext &ctx, allocator_arg_t, StackAllocator &&stack_allocator,
...
void spawn(
    ...
    const boost::coroutines::attributes &attributes,
    constraint_t<!is_executor<decay_t<Handler>>::value &&
  ...
  (dispatch)(
      ...
      detail::spawned_thread_resumer(detail::spawned_coroutine_thread::spawn(
          detail::old_spawn_entry_point<executor_type, decay_t<Function>,
                                        void (*)()>(
              ex, static_cast<Function &&>(function),
              &detail::default_spawn_handler),
          attributes)));
}
...
void spawn(basic_yield_context<Executor> ctx, Function &&function,
  ...
  (dispatch)(
      ctx.get_executor(),
      detail::spawned_thread_resumer(detail::spawned_coroutine_thread::spawn(
          detail::old_spawn_entry_point<Executor, decay_t<Function>,
                                        void (*)()>(
...
inline void spawn(const Executor &ex, Function &&function,
                  ...
                  constraint_t<is_executor<Executor>::value ||
                               execution::is_executor<Executor>::value>) {
  asio::spawn(asio::strand<Executor>(ex), static_cast<Function &&>(function),
              attributes);
}
...
inline void
spawn(ExecutionContext &ctx, Function &&function,
      const boost::coroutines::attributes &attributes,
      constraint_t<
          is_convertible<ExecutionContext &, execution_context &>::value>) {
  asio::spawn(ctx.get_executor(), static_cast<Function &&>(function),
              attributes);
}
