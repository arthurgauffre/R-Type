...
template <typename Clock, typename Duration>
std::size_t
io_context::run_one_until(const chrono::time_point<Clock, Duration> &abs_time) {
...
struct io_context::initiate_dispatch {
  ...
  void operator()(LegacyCompletionHandler &&handler, io_context *self) const {
    ...
    if (self->impl_.can_dispatch()) {
      ...
      typedef detail::completion_handler<decay_t<LegacyCompletionHandler>,
                                         executor_type>
          op;
...
auto io_context::dispatch(LegacyCompletionHandler &&handler)
    -> decltype(async_initiate<LegacyCompletionHandler, void()>(
        declval<initiate_dispatch>(), handler, this)) {
  return async_initiate<LegacyCompletionHandler, void()>(initiate_dispatch(),
                                                         handler, this);
}
...
auto io_context::post(LegacyCompletionHandler &&handler)
  ...
  return async_initiate<LegacyCompletionHandler, void()>(initiate_post(),
                                                         handler, this);
}
...
template <typename Allocator, uintptr_t Bits>
inline bool
io_context::basic_executor_type<Allocator, Bits>::running_in_this_thread()
    const noexcept {
...
void io_context::basic_executor_type<Allocator, Bits>::execute(
  ...
  p.p = new (p.v)
      op(static_cast<Function &&>(f), static_cast<const Allocator &>(*this));

  ASIO_HANDLER_CREATION(
      (*context_ptr(), *p.p, "io_context", context_ptr(), 0, "execute"));

  context_ptr()->impl_.post_immediate_completion(
      p.p, (bits() & relationship_continuation) != 0);
  p.v = p.p = 0;
}
...
template <typename Allocator, uintptr_t Bits>
inline io_context &
io_context::basic_executor_type<Allocator, Bits>::context() const noexcept {
  return *context_ptr();
}

template <typename Allocator, uintptr_t Bits>
inline void io_context::basic_executor_type<Allocator, Bits>::on_work_started()
    const noexcept {
  context_ptr()->impl_.work_started();
}

template <typename Allocator, uintptr_t Bits>
inline void io_context::basic_executor_type<Allocator, Bits>::on_work_finished()
    const noexcept {
...
void io_context::basic_executor_type<Allocator, Bits>::dispatch(
  ...
  // Allocate and construct an operation to wrap the function.
  typedef detail::executor_op<function_type, OtherAllocator, detail::operation>
      op;
  typename op::ptr p = {detail::addressof(a), op::ptr::allocate(a), 0};
  p.p = new (p.v) op(static_cast<Function &&>(f), a);

  ASIO_HANDLER_CREATION(
      (*context_ptr(), *p.p, "io_context", context_ptr(), 0, "dispatch"));
...
void io_context::basic_executor_type<Allocator, Bits>::post(
    Function &&f, const OtherAllocator &a) const {
  // Allocate and construct an operation to wrap the function.
  typedef detail::executor_op<decay_t<Function>, OtherAllocator,
                              detail::operation>
      op;
  typename op::ptr p = {detail::addressof(a), op::ptr::allocate(a), 0};
  p.p = new (p.v) op(static_cast<Function &&>(f), a);
  ...
  ASIO_HANDLER_CREATION(
      (*context_ptr(), *p.p, "io_context", context_ptr(), 0, "post"));
...
void io_context::basic_executor_type<Allocator, Bits>::defer(
  ...
  ASIO_HANDLER_CREATION(
      (*context_ptr(), *p.p, "io_context", context_ptr(), 0, "defer"));
