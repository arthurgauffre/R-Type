...
public:
  ...
  template <typename... Args> void operator()(Args &&...args) {
    this->invoke(index_sequence_for<Values...>{},
...
inline bool
asio_handler_is_continuation(prepend_handler<Handler> *this_handler) {
...
struct async_result<prepend_t<CompletionToken, Values...>, Signature>
    : async_result<CompletionToken, typename detail::prepend_signature<
                                        Signature, Values...>::type> {
  typedef
      typename detail::prepend_signature<Signature, Values...>::type signature;
  ...
  template <typename Initiation> struct init_wrapper {
    ...
    void operator()(Handler &&handler, std::tuple<Values...> values,
                    Args &&...args) {
  ...
  static auto initiate(Initiation &&initiation, RawCompletionToken &&token,
      ...
      -> decltype(async_initiate<CompletionToken, signature>(
          declval<init_wrapper<decay_t<Initiation>>>(), token.token_,
...
template <template <typename, typename> class Associator, typename Handler,
          typename... Values, typename DefaultCandidate>
struct associator<Associator, detail::prepend_handler<Handler, Values...>,
                  DefaultCandidate> : Associator<Handler, DefaultCandidate> {
  static typename Associator<Handler, DefaultCandidate>::type
  get(const detail::prepend_handler<Handler, Values...> &h) noexcept {
