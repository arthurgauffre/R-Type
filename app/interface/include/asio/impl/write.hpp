...
std::size_t write(SyncWriteStream &s, const ConstBufferSequence &buffers,
                  ...
                  asio::error_code &ec) {
  ec = asio::error_code();
  asio::detail::consuming_buffers<const_buffer, ConstBufferSequence,
                                  ConstBufferIterator>
      tmp(buffers);
  while (!tmp.empty()) {
...
write(SyncWriteStream &s, const ConstBufferSequence &buffers,
  ...
  return detail::write(
      s, buffers, asio::buffer_sequence_begin(buffers),
...
template <typename SyncWriteStream, typename DynamicBuffer_v1,
          typename CompletionCondition>
std::size_t
write(SyncWriteStream &s, DynamicBuffer_v1 &&buffers,
      CompletionCondition completion_condition, asio::error_code &ec,
...
write(SyncWriteStream &s, DynamicBuffer_v1 &&buffers,
  ...
  asio::error_code ec;
  std::size_t bytes_transferred =
      write(s, static_cast<DynamicBuffer_v1 &&>(buffers), transfer_all(), ec);
  asio::detail::throw_error(ec, "write");
  return bytes_transferred;
}
...
template <typename SyncWriteStream, typename DynamicBuffer_v1>
inline std::size_t
write(SyncWriteStream &s, DynamicBuffer_v1 &&buffers, asio::error_code &ec,
      constraint_t<is_dynamic_buffer_v1<decay_t<DynamicBuffer_v1>>::value>,
      constraint_t<!is_dynamic_buffer_v2<decay_t<DynamicBuffer_v1>>::value>) {
  return write(s, static_cast<DynamicBuffer_v1 &&>(buffers), transfer_all(),
               ec);
}
...
write(SyncWriteStream &s, DynamicBuffer_v1 &&buffers,
  ...
  asio::error_code ec;
  std::size_t bytes_transferred =
      write(s, static_cast<DynamicBuffer_v1 &&>(buffers),
            static_cast<CompletionCondition &&>(completion_condition), ec);
  asio::detail::throw_error(ec, "write");
  return bytes_transferred;
}
...
template <typename SyncWriteStream, typename Allocator,
          typename CompletionCondition>
inline std::size_t
write(SyncWriteStream &s, asio::basic_streambuf<Allocator> &b,
      CompletionCondition completion_condition, asio::error_code &ec) {
...
write(SyncWriteStream &s, DynamicBuffer_v2 buffers,
  ...
  asio::error_code ec;
  std::size_t bytes_transferred =
      write(s, static_cast<DynamicBuffer_v2 &&>(buffers), transfer_all(), ec);
  asio::detail::throw_error(ec, "write");
  return bytes_transferred;
}
...
write(SyncWriteStream &s, DynamicBuffer_v2 buffers, asio::error_code &ec,
      constraint_t<is_dynamic_buffer_v2<DynamicBuffer_v2>::value>) {
  return write(s, static_cast<DynamicBuffer_v2 &&>(buffers), transfer_all(),
               ec);
}
...
write(SyncWriteStream &s, DynamicBuffer_v2 buffers,
  ...
  asio::error_code ec;
  std::size_t bytes_transferred =
      write(s, static_cast<DynamicBuffer_v2 &&>(buffers),
            static_cast<CompletionCondition &&>(completion_condition), ec);
  asio::detail::throw_error(ec, "write");
  return bytes_transferred;
}
...
class write_op : public base_from_cancellation_state<WriteHandler>,
...
public:
  write_op(AsyncWriteStream &stream, const ConstBufferSequence &buffers,
      ...
      : base_from_cancellation_state<WriteHandler>(
        ...
        stream_(stream), buffers_(buffers), start_(0),
  ...
  // private:
  typedef asio::detail::consuming_buffers<const_buffer, ConstBufferSequence,
                                          ConstBufferIterator>
      buffers_type;
...
public:
  ...
  void operator()(WriteHandler &&handler, const ConstBufferSequence &buffers,
    ...
    start_write_op(stream_, buffers, asio::buffer_sequence_begin(buffers),
...
struct associator<
  ...
  get(const detail::write_op<AsyncWriteStream, ConstBufferSequence,
                             ConstBufferIterator, CompletionCondition,
                             WriteHandler> &h) noexcept {
    return Associator<WriteHandler, DefaultCandidate>::get(h.handler_);
  }

  static auto get(const detail::write_op<AsyncWriteStream, ConstBufferSequence,
                                         ConstBufferIterator,
                                         CompletionCondition, WriteHandler> &h,
                  const DefaultCandidate &c) noexcept
      -> decltype(Associator<WriteHandler, DefaultCandidate>::get(h.handler_,
                                                                  c)) {
...
public:
  ...
  write_dynbuf_v1_op(AsyncWriteStream &stream, BufferSequence &&buffers,
                     CompletionCondition &completion_condition,
                     WriteHandler &handler)
  ...
  write_dynbuf_v1_op(write_dynbuf_v1_op &&other)
      : stream_(other.stream_),
        ...
        completion_condition_(
            static_cast<CompletionCondition &&>(other.completion_condition_)),
        handler_(static_cast<WriteHandler &&>(other.handler_)) {}
  ...
  void operator()(const asio::error_code &ec, std::size_t bytes_transferred,
    ...
    default:
      buffers_.consume(bytes_transferred);
      static_cast<WriteHandler &&>(handler_)(
          ec, static_cast<const std::size_t &>(bytes_transferred));
    }
  }
...
template <typename AsyncWriteStream, typename DynamicBuffer_v1,
          typename CompletionCondition, typename WriteHandler>
inline bool asio_handler_is_continuation(
    write_dynbuf_v1_op<AsyncWriteStream, DynamicBuffer_v1, CompletionCondition,
...
public:
  ...
  template <typename WriteHandler, typename DynamicBuffer_v1,
            typename CompletionCondition>
  void operator()(WriteHandler &&handler, DynamicBuffer_v1 &&buffers,
                  CompletionCondition &&completion_cond) const {
    // If you get an error on the following line it means that your handler
    // does not meet the documented type requirements for a WriteHandler.
    ASIO_WRITE_HANDLER_CHECK(WriteHandler, handler) type_check;
    ...
    non_const_lvalue<WriteHandler> handler2(handler);
    non_const_lvalue<CompletionCondition> completion_cond2(completion_cond);
    write_dynbuf_v1_op<AsyncWriteStream, decay_t<DynamicBuffer_v1>,
                       CompletionCondition, decay_t<WriteHandler>>(
        stream_, static_cast<DynamicBuffer_v1 &&>(buffers),
        completion_cond2.value, handler2.value)(asio::error_code(), 0, 1);
  }
...
struct associator<Associator,
                  detail::write_dynbuf_v1_op<AsyncWriteStream, DynamicBuffer_v1,
                                             CompletionCondition, WriteHandler>,
                  DefaultCandidate>
...
inline auto async_write(
    AsyncWriteStream &s, DynamicBuffer_v1 &&buffers, WriteToken &&token,
    constraint_t<is_dynamic_buffer_v1<decay_t<DynamicBuffer_v1>>::value>,
    constraint_t<!is_dynamic_buffer_v2<decay_t<DynamicBuffer_v1>>::value>)
    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
  ...
  return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
      detail::initiate_async_write_dynbuf_v1<AsyncWriteStream>(s), token,
...
inline auto async_write(AsyncWriteStream &s,
                        asio::basic_streambuf<Allocator> &b, WriteToken &&token)
    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
        declval<detail::initiate_async_write_dynbuf_v1<AsyncWriteStream>>(),
        token, basic_streambuf_ref<Allocator>(b), transfer_all())) {
  return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
      detail::initiate_async_write_dynbuf_v1<AsyncWriteStream>(s), token,
      basic_streambuf_ref<Allocator>(b), transfer_all());
}
...
public:
  template <typename BufferSequence>
  write_dynbuf_v2_op(AsyncWriteStream &stream, BufferSequence &&buffers,
                     CompletionCondition &completion_condition,
                     WriteHandler &handler)
      : stream_(stream), buffers_(static_cast<BufferSequence &&>(buffers)),
        completion_condition_(
...
template <typename AsyncWriteStream, typename DynamicBuffer_v2,
          typename CompletionCondition, typename WriteHandler>
inline bool asio_handler_is_continuation(
    write_dynbuf_v2_op<AsyncWriteStream, DynamicBuffer_v2, CompletionCondition,
                       WriteHandler> *this_handler) {
  return asio_handler_cont_helpers::is_continuation(this_handler->handler_);
}
...
public:
  ...
  template <typename WriteHandler, typename DynamicBuffer_v2,
            typename CompletionCondition>
  void operator()(WriteHandler &&handler, DynamicBuffer_v2 &&buffers,
                  CompletionCondition &&completion_cond) const {
    // If you get an error on the following line it means that your handler
    // does not meet the documented type requirements for a WriteHandler.
    ASIO_WRITE_HANDLER_CHECK(WriteHandler, handler) type_check;
...
template <template <typename, typename> class Associator,
          ...
          typename DefaultCandidate>
struct associator<Associator,
                  detail::write_dynbuf_v2_op<AsyncWriteStream, DynamicBuffer_v2,
                                             CompletionCondition, WriteHandler>,
                  DefaultCandidate>
    : Associator<WriteHandler, DefaultCandidate> {
  static typename Associator<WriteHandler, DefaultCandidate>::type
...
template <typename AsyncWriteStream, typename DynamicBuffer_v2,
          ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, std::size_t))
              WriteToken>
inline auto
async_write(AsyncWriteStream &s, DynamicBuffer_v2 buffers, WriteToken &&token,
            constraint_t<is_dynamic_buffer_v2<DynamicBuffer_v2>::value>)
    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
        declval<detail::initiate_async_write_dynbuf_v2<AsyncWriteStream>>(),
        token, static_cast<DynamicBuffer_v2 &&>(buffers), transfer_all())) {
  return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
      detail::initiate_async_write_dynbuf_v2<AsyncWriteStream>(s), token,
