...
public:
  ...
  void operator()(WriteHandler &&handler,
    ...
    async_write(next_layer_, buffer(storage->data(), storage->size()),
                buffered_flush_handler<decay_t<WriteHandler>>(*storage,
                                                              handler2.value));
  }
...
template <template <typename, typename> class Associator, typename WriteHandler,
          typename DefaultCandidate>
struct associator<Associator, detail::buffered_flush_handler<WriteHandler>,
                  DefaultCandidate>
    : Associator<WriteHandler, DefaultCandidate> {
  static typename Associator<WriteHandler, DefaultCandidate>::type
  get(const detail::buffered_flush_handler<WriteHandler> &h) noexcept {
...
inline auto buffered_write_stream<Stream>::async_flush(WriteHandler &&handler)
    -> decltype(async_initiate<WriteHandler,
  ...
  return async_initiate<WriteHandler, void(asio::error_code, std::size_t)>(
      detail::initiate_async_buffered_flush<Stream>(next_layer_), handler,
      &storage_);
}
...
public:
  ...
  void operator()(WriteHandler &&handler, buffered_stream_storage *storage,
    ...
    if (buffer_size(buffers) == 0 || storage->size() < storage->capacity()) {
      next_layer_.async_write_some(
          ...
          buffered_write_some_handler<ConstBufferSequence,
                                      decay_t<WriteHandler>>(*storage, buffers,
                                                             handler2.value));
    } else {
...
struct associator<
  ...
  static auto get(const detail::buffered_write_some_handler<ConstBufferSequence,
                                                            WriteHandler> &h,
                  const DefaultCandidate &c) noexcept
      -> decltype(Associator<WriteHandler, DefaultCandidate>::get(h.handler_,
                                                                  c)) {
...
inline auto buffered_write_stream<Stream>::async_write_some(
  ...
  return async_initiate<WriteHandler, void(asio::error_code, std::size_t)>(
      detail::initiate_async_buffered_write_some<Stream>(next_layer_), handler,
      &storage_, buffers);
}
