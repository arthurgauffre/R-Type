...
public:
  ...
  cancellation_handler(std::size_t size, Args &&...args)
      : handler_(static_cast<Args &&>(args)...), size_(size) {}
...
public:
  ...
  CancellationHandler &emplace(Args &&...args) {
    ...
    auto_delete_helper del = {prepare_memory(sizeof(cancellation_handler_type),
                                             alignof(CancellationHandler))};
    cancellation_handler_type *handler_obj = new (del.mem.first)
        cancellation_handler_type(del.mem.second,
...
private:
  ...
  ASIO_DECL std::pair<void *, std::size_t> prepare_memory(std::size_t size,
                                                          std::size_t align);
