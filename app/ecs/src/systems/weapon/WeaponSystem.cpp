                                                ...
                                                int damage)
{
    ...
    sf::Vector2f position = {
        transformPlayer->getPosition().x + texturePlayer->getSize().x * transformPlayer->getScale().x,
        transformPlayer->getPosition().y + (texturePlayer->getSize().y * transformPlayer->getScale().y) / 2};

    if (_componentManager.getComponent<component::TypeComponent>(parentID)->getType() == "enemy")
        position.x -= texturePlayer->getSize().x * transformPlayer->getScale().x;

    component::TransformComponent *transformComponent = _componentManager.addComponent<component::TransformComponent>(projectileID, position, scale);
    _componentManager.addComponent<component::PositionComponent>(projectileID, position.x, position.y);
    _componentManager.addComponent<component::SpriteComponent>(projectileID, position.x, position.y);
...
void ECS_system::WeaponSystem::update(float deltaTime, std::vector<std::shared_ptr<entity::IEntity>> entities)
{
    for (auto &entity : _componentManager.getEntitiesWithComponents<component::WeaponComponent>(entities))
    {
        ...
        std::string entityType = _componentManager.getComponent<component::TypeComponent>(entity->getID())->getType();

        if (weaponComponent->getIsFiring() && cooldownComponent->getTimeRemaining() > 0)
        ...
        {
            sf::Vector2f projectileVelocity = (entityType == "player") ? sf::Vector2f(200, 0) : sf::Vector2f(-200, 0);
